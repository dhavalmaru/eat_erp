...
class Login extends CI_Controller
{
    ...
    public function check_credentials() {
        ...
        $query=$this->db->query("SELECT * FROM user_master WHERE email_id = '$uname' AND password = '$upass'");
        $result=$query->result();
    ...
    public function forgot_password_email() {
		...
		if($to_email!=''){
			if (count($result) > 0) {
				$from_email = 'vaibhav.desai@eatanytime.in';
    ...
    function send_password_email() {
        try {
            $from_email = 'vaibhav.desai@eatanytime.in'; //change this to yours
            ...
            if (count($result) > 0) {
                ...
                $config['smtp_pass'] = 'Vaibhav@12345'; //$from_email password
