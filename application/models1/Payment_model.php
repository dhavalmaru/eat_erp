...
function get_data($status='', $id=''){
    ...
    $sql = "select C.*, D.b_name, D.b_branch from 
            (select * from payment_details".$cond.") C 
            left join 
            (select * from bank_master) D 
            on (C.bank_id=D.id) order by C.modified_on desc";
...
function get_total_outstanding($id, $distributor_id, $module){
    ...
    $sql = "select A.distributor_id, sum((ifnull(A.total_amount,0)-ifnull(C.paid_amount,0))) as total_outstanding from 
            (select distributor_id, sum(final_amount) as total_amount from distributor_out 
...
function get_invoice_nos($id, $distributor_id){
    $sql = "select * from 
            (select A.distributor_id, A.invoice_no, (ifnull(A.final_amount,0)-ifnull(B.tot_payment_amount,0)) as bal_amount from 
...
function get_invoice_details($id, $invoice_no){
    $sql = "select * from 
            (select A.distributor_id, A.invoice_no, A.client_name, 
                (ifnull(A.final_amount,0)-ifnull(B.tot_payment_amount,0)) as final_amount from 
