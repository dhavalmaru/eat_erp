...
function get_distributor_out_details($from_date, $to_date) {
    $sql = "select I.*, J.item_name from 
            (select G.*, H.sales_rep_name from 
            (select E.*, F.distributor_name, F.sell_out from 
...
function generate_sale_invoice_report() {
    ...
    if(count($data)>0) {
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('J'.$row, $data[$i]->cst_amount);
            $objPHPExcel->getActiveSheet()->setCellValue('K'.$row, $data[$i]->final_amount);
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('N'.$row, $data[$i]->sales_rep_name);
            $objPHPExcel->getActiveSheet()->setCellValue('O'.$row, $data[$i]->due_date);
            $objPHPExcel->getActiveSheet()->setCellValue('P'.$row, $data[$i]->order_no);
            $objPHPExcel->getActiveSheet()->setCellValue('Q'.$row, $data[$i]->order_date);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1'.':Q'.$row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
...
function generate_distributor_ledger_report() {
    ...
    $sql = "select date_of_processing as ref_date, invoice_no as reference, final_amount as debit_amount, null as credit_amount 
                from distributor_out where status = 'Approved' and distributor_id = '$distributor_id' and 
                date_of_processing>='$from_date' and date_of_processing<='$to_date' 
            union all 
            select date_of_processing as ref_date, null as reference, null as debit_amount, final_amount as credit_amount 
                from distributor_in where status = 'Approved' and distributor_id = '$distributor_id' and 
                date_of_processing>='$from_date' and date_of_processing<='$to_date' 
            union all 
            select * from 
            (select A.date_of_deposit as ref_date, B.invoice_no as reference, null as debit_amount, B.payment_amount as credit_amount from 
            ...
            union all 
            select date_of_transaction as ref_date, transaction as reference, 
                    case when transaction='Debit Note' then amount end as debit_amount, 
                    case when transaction='Credit Note' then amount end as credit_amount 
                from credit_debit_note where status = 'Approved' and distributor_id = '$distributor_id' and 
                date_of_transaction>='$from_date' and date_of_transaction<='$to_date'";
    $query=$this->db->query($sql);
    $data=$query->result();
    ...
    if(count($data)>0) {
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1:'.$col_name[$col+3].$row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
...
function generate_distributorwise_report() {
    ...
    if(count($data)>0) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+19].$row, "Contact Person *");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+20].$row, "Email Id");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+21].$row, "Mobile *");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+22].$row, "Status");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+23].$row, "Modified On");
        ...
        for($i=0; $i<count($data); $i++){
            ...
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+19].$row, $data[$i]->c_contact_person);
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+20].$row, $data[$i]->c_email_id);
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+21].$row, $data[$i]->c_mobile);
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+22].$row, $data[$i]->d_status);
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+23].$row, $data[$i]->modified_date);
        }
