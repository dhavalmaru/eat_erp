...
class Dashboard_model extends CI_Model {
    ...
    function get_total_sale() {
        $sql="select ifnull(C.total_amount,0)-ifnull(D.total_amount,0) as total_amount, ifnull(C.total_bar,0)-ifnull(D.total_bar,0) as total_bar, 
            ifnull(C.total_box,0)-ifnull(D.total_box,0) as total_box from 
            (select '1' as temp_col, round(sum(A.final_amount),0) as total_amount, sum(B.bar_qty) as total_bar, sum(B.box_qty) as total_box from 
            (select id, final_amount from distributor_out where status = 'Approved') A 
            left join 
            ...
            left join 
            (select '1' as temp_col, round(sum(A.final_amount),0) as total_amount, sum(B.bar_qty) as total_bar, sum(B.box_qty) as total_box from 
            (select id, final_amount from distributor_in where status = 'Approved') A 
            left join 
    ...
    function get_total_distributor() {
        $sql="select count(A.id) as tot_dist, sum(A.g_trade) as tot_g_trade, sum(A.m_trade) as tot_m_trade, 
            sum(A.e_com) as tot_e_com from (select id, case when class='General Trade' then 1 else 0 end as g_trade, 
            case when class='Modern Trade' then 1 else 0 end as m_trade, 
            case when class='E Commerce' then 1 else 0 end as e_com 
            from distributor_master where status = 'Approved') A";
        $query=$this->db->query($sql);
        return $query->result();
    }
    ...
    function get_total_receivable() {
        $sql="select ifnull(E.total_amount,0)-ifnull(G.total_amount,0) as total_receivable from 
            (select '1' as temp_col, ifnull(C.total_amount,0)-ifnull(D.total_amount,0) as total_amount, 
                ifnull(C.total_bar,0)-ifnull(D.total_bar,0) as total_bar, 
            ifnull(C.total_box,0)-ifnull(D.total_box,0) as total_box from 
            (select '1' as temp_col, round(sum(A.final_amount),0) as total_amount, sum(B.bar_qty) as total_bar, 
                sum(B.box_qty) as total_box from 
            (select id, final_amount from distributor_out where status = 'Approved') A 
            left join 
            (select distributor_out_id, case when type='Bar' then qty else 0 end as bar_qty, 
            case when type='Box' then qty else 0 end as box_qty from distributor_out_items) B 
            ...
            case when type='Box' then qty else 0 end as box_qty from distributor_in_items) B 
            on (A.id=B.distributor_in_id)) D 
            on (C.temp_col=D.temp_col)) E 
            left join 
            (select '1' as temp_col, sum(total_amount) as total_amount from payment_details where status = 'Approved') G 
            on (E.temp_col=G.temp_col)";
        $query=$this->db->query($sql);
        return $query->result();
    }
    ...
    function get_avg_total_sale_in_rs($from_date, $to_date) {
        $sql="select distinct C.date_of_processing, C.total_amount from (select A.date_of_processing, 
            (select avg(B.amount) from distributor_out B 
            where B.status = 'Approved' and B.date_of_processing<=A.date_of_processing) as total_amount 
            from distributor_out A where A.status = 'Approved' and date(A.date_of_processing) >= date('" . $from_date . "') and 
    ...
    function get_avg_total_sale($from_date, $to_date, $type) {
        $sql="select distinct C.date_of_processing, C.total_qty from (select A.date_of_processing, (select avg(B.qty) from (select * from 
    ...
    function get_product_wise_sale_in_bar($from_date, $to_date) {
        $sql="select D.item_id, E.product_name, D.total_qty from 
            ...
            (select id, monthname(date_of_processing) as month_name from distributor_out where status = 'Approved' and 
                        date(date_of_processing) >= date('" . $from_date . "') and 
                        date(date_of_processing) <= date('" . $to_date . "')) A 
            left join 
            (select distributor_out_id, item_id, qty from distributor_out_items where type = 'Bar') B 
            on (A.id=B.distributor_out_id)) C 
    ...
    function get_months($from_date, $to_date) {
        $sql="select * from 
            (select distinct month(date_of_processing) as month_no, monthname(date_of_processing) as month_name 
            from distributor_out where status = 'Approved' and 
                        date(date_of_processing) >= date('" . $from_date . "') and 
                        date(date_of_processing) <= date('" . $to_date . "')) A order by A.month_no";
        $query=$this->db->query($sql);
        return $query->result();
    }
    ...
    function get_month_wise_sale_in_box($from_date, $to_date) {
        $sql="select D.item_id, D.month_no, D.month_name, E.box_name, D.total_qty from 
            ...
            (select id, month(date_of_processing) as month_no, monthname(date_of_processing) as month_name from distributor_out 
            ...
            left join 
            (select * from box_master) E 
            on (D.item_id = E.id) order by D.item_id, D.month_no";
        $query=$this->db->query($sql);
        return $query->result();
    }
    ...
    function get_sm_wise_sale_in_bar($from_date, $to_date) {
        $sql="select D.sales_rep_id, E.sales_rep_name, D.total_qty from 
            ...
            (select id, sales_rep_id, monthname(date_of_processing) as month_name from distributor_out where status = 'Approved' and 
                        date(date_of_processing) >= date('" . $from_date . "') and 
                        date(date_of_processing) <= date('" . $to_date . "')) A 
            left join 
            (select distributor_out_id, item_id, qty from distributor_out_items where type = 'Bar') B 
    ...
    function get_category_wise_sale_in_bar($from_date, $to_date) {
        $sql="select E.type_id, E.distributor_type, sum(E.qty) as total_qty from 
            ...
            (select id, distributor_id, monthname(date_of_processing) as month_name from distributor_out where status = 'Approved' and 
                        date(date_of_processing) >= date('" . $from_date . "') and 
                        date(date_of_processing) <= date('" . $to_date . "')) A 
            left join 
            (select distributor_out_id, item_id, qty from distributor_out_items where type = 'Bar') B 
    ...
    function get_category_month_wise_sale_in_bar($from_date, $to_date) {
        $sql="select E.type_id, E.month_no, E.month_name, E.distributor_type, sum(E.qty) as total_qty from 
            ...
            (select id, distributor_id, month(date_of_processing) as month_no, monthname(date_of_processing) as month_name from distributor_out where status = 'Approved' and 
                        date(date_of_processing) >= date('" . $from_date . "') and 
                        date(date_of_processing) <= date('" . $to_date . "')) A 
            left join 
            (select distributor_out_id, item_id, qty from distributor_out_items where type = 'Bar') B 
