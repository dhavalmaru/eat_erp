...
class Mt_stock extends CI_Controller{
    ...
    public function __construct(){
        ...
        $this->load->model('mt_stock_model');
    ...
    public function index(){
        $result=$this->mt_stock_model->get_access();
        if(count($result)>0) {
            $data['access']=$result;
            $data['data'] = $this->mt_stock_model->get_data();
    ...
    // public function get_store_locations($id){
        // $result=$this->mt_stock_model->get_access();
        // if(count($result)>0) {
			      // $query = $this->mt_stock_model->get_data('', $id);
            ...
            // if(count($query)>0) {
                ...
                // }
            // }
		// $data['data'] = $this->mt_stock_model->get_data('', $id);
    ...
    public function get_store_locations(){
        ...
        $result=$this->mt_stock_model->get_data('', $id);
        $data['result'] = 0;
        if(count($result)>0) {
            $data['result'] = 1;
    ...
    public function add(){
        ...
        if(count($result)>0) {
            if($result[0]->r_insert == 1) {
                $data['access'] = $this->mt_stock_model->get_access();
    ...
    public function save(){
        $this->mt_stock_model->save_data();
        redirect(base_url().'index.php/store');
    }

    public function update($id){
        $this->mt_stock_model->save_data($id);
        redirect(base_url().'index.php/store');
    }
