...
class Distributor_out extends CI_Controller{
    ...
    public function checkstatus($status=''){
        ...
        if(count($result)>0) {
            ...
            $data['data']=$this->distributor_out_model->get_distributor_out_data($status);

            $count_data=$this->distributor_out_model->get_distributor_out_data();
            $active=0;
            $inactive=0;
            $pending=0;
            ...
            if (count($result)>0){
                for($i=0;$i<count($count_data);$i++){
                    ...
                    if (strtoupper(trim($count_data[$i]->status))=="APPROVED" && strtoupper(trim($count_data[$i]->delivery_status))=="PENDING")
    ...
    public function set_delivery_status(){
        $this->tax_invoice_model->set_delivery_status();
