    ...
    public function add($id){ 
        ...
        if(count($result)>0) {
            if($result[0]->r_insert == 1) {
                ...
                $sql = "Select max(sequence) as sequence from merchandiser_detailed_beat_plan WHERE date(date_of_visit)=date(now()) and is_edit='edit' and frequency='$frequency'";
                $get_maxcount = $this->db->query($sql)->result_array();

                if($get_maxcount[0]['sequence']=='')
                    $data['sequence_count'] = '';
                else
