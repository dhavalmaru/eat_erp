   ...
   public function get_sales_rep_byemail($value='')
    {
        ...
        $sql = "Select  A.*,E.p_call,D.actual_count,Case When F.check_in_time IS NULL THEN 'Absent' Else 'Present' end as emp_status
            ...
            ) V on A.sales_rep_id=V.sales_rep_id
            LEFT JOIN ( 
                SELECT count(*) as actual_count,sales_rep_id from sales_rep_beat_plan Where frequency= '$frequency' Group By sales_rep_id
            ) D 
            on D.sales_rep_id=A.sales_rep_id
        ...
        $sql2 = "Select  sum((Case When A.unplanned_count IS NULL THEN 0  Else A.unplanned_count end)) as unplanned_count ,((sum(Case When A.unplanned_count IS NULL THEN 0  Else A.unplanned_count end))+sum((Case When A.planned_count IS NULL THEN 0 Else A.planned_count end ))) as total_visit ,Sum(Case When F.check_in_time IS NULL THEN 1 Else 0  end) as absent,Sum(Case When F.check_in_time IS NOT  NULL THEN 1 Else 0  end) as present,sum(Case WHEN O.od_units IS NULL Then 0 Else O.od_units end) as od_units,'' as od_value ,sum((Case When A.planned_count IS NULL THEN 0  Else A.planned_count end)) as planned_count,sum(actual_count) as actual_count
        from (
        SELECT A.sales_rep_name,A.id,B.planned_count,C.unplanned_count,C.sales_rep_id,C.zone_id From
        (SELECT * from sales_rep_master Where status='Approved' and sr_type='Sales Representative')A
        Left join
                            ...
                            <td class="innerpadding " style="padding: 30px 30px 30px 30px;" >
                               <table class="table upper_table" style="width:765px;border-collapse: collapse;margin: 0 auto;" >
                                    <tr>
                                        ...
                                        if(count($data2)>0)
                                        {
                                            $tbody.=($data2[0]->actual_count==''?0:$data2[0]->actual_count);
    ...
    public function get_promoter_byemail($value='')
    {
         $sql = "Select CASE WHEN ((FLOOR((DayOfMonth(date(now()))-1)/7)+1 )=1 OR (FLOOR((DayOfMonth(date(now()))-1)/7)+1 )=3 
            ...
            THEN  CONCAT('Alternate ',DAYNAME(date(now()))) end  as frequency";
        $result = $this->db->query($sql)->result();
        echo $frequency = $result[0]->frequency;
        $sql = "Select  A.*,E.p_call,D.actual_count,Case When F.check_in_time IS NULL THEN 'Absent' Else 'Present' end as emp_status
            ,O.od_units,V.od_value
            from (
            SELECT A.sales_rep_name,A.id,B.planned_count,C.unplanned_count,C.sales_rep_id,C.zone_id,A.zone From
            ...
            Left join
            (SELECT count(*) as planned_count,sales_rep_id,zone_id from merchandiser_beat_plan A Where store_id 
            In ( Select store_id from merchandiser_detailed_beat_plan Where frequency='$frequency'  and sales_rep_id=A.sales_rep_id and is_edit='edit' and date(date_of_visit)=date(now()) and bit_plan_id!=0) 
            and date(created_on)<>date(now()) and frequency='$frequency' GROUP By sales_rep_id,zone_id) B On A.id=B.sales_rep_id
            Left join
            (
                SELECT  sum(unplanned_count) as unplanned_count,sales_rep_id,zone_id from 
            ...
            LEFT JOIN ( 
                SELECT count(*) as actual_count,sales_rep_id from merchandiser_beat_plan Where frequency= '$frequency' Group By sales_rep_id
            ) D 
            on D.sales_rep_id=A.sales_rep_id
            Left Join 
            (Select * from sales_attendence Where date(check_in_time)=date(now())) F On A.id=F.sales_rep_id
       ...
       $to_email = "rishit.sanghvi@eatanytime.in, dhaval.maru@pecanreams.com, swapnil.darekar@eatanytime.in,mukesh@eatanytime.in,sulochana@eatanytime.co.in";
        // $to_email = "ashwini.patil@pecanreams.com, dhaval.maru@pecanreams.com";
        // $bcc = "sangeeta.yadav@pecanreams.com";*/
