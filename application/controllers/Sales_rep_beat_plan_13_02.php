   ...
   public function get_sales_rep_byemail($value='')
	{
		...
		$sql = "Select  A.*,E.p_call,D.actual_count,Case When (F.check_in_time IS NULL OR F.working_status='Absent') THEN 'Absent' Else 'Present' end as emp_status
			,O.od_units,V.od_value
			from (
			SELECT A.sales_rep_name,A.id,B.planned_count,C.unplanned_count,C.sales_rep_id,C.zone_id,A.zone From
			(SELECT * from sales_rep_master Where status='Approved' and sr_type='Sales Representative')A
			Left join
			(SELECT count(*) as planned_count,sales_rep_id from sales_rep_beat_plan A Where date(created_on)<>date(now()) and frequency='$frequency' GROUP By sales_rep_id) B On A.id=B.sales_rep_id
			Left join
			(
				SELECT  sum(unplanned_count) as unplanned_count,sales_rep_id,zone_id from 
				(SELECT count(*) as unplanned_count,sales_rep_id,zone_id from sales_rep_beat_plan A Where store_id 
			...
			(Select * from zone_master Where status='Approved') Z On A.zone_id=Z.id
			left join 
			(SELECT count(id) as p_call,sales_rep_id from sales_rep_orders Where id IN (Select sales_rep_order_id From sales_rep_order_items) and date(created_on)=date(now())) E 
			On A.id=E.sales_rep_id
			LEFT Join
			(select sum(B.item_qty)as od_units,sales_rep_id from 
			(select * from sales_rep_orders where date(date_of_processing) = date(now()) )A 
			left join (Select case when E.type='Bar' then E.qty when (E.type='Box' and (E.item_id IN (4,26,27,13,14,32,37,38,39,43,44,45))) then E.qty else (E.qty*F.qty) end as item_qty,sales_rep_order_id from 
			...
			Group by sales_rep_order_id) B on (A.id=B.sales_rep_order_id)
			GROUP By sales_rep_id) O on A.id=O.sales_rep_id
			LEFT JOIN
			(SELECT sum( Case When rate is not null Then (rate*E.qty) else 0 end ) as od_value,sales_rep_id from 
				(SELECT * from sales_rep_orders WHERE date(date_of_processing)=date(now())) A
				Left join 
			...
			LEFT JOIN ( 
				 SELECT count(*) as actual_count ,sales_rep_id from ( 
							SELECT store_id,sales_rep_id,date_of_visit from sales_rep_detailed_beat_plan A 
							Where frequency= '".$frequency."' and store_id IN (Select DISTINCT store_id from sales_rep_beat_plan Where frequency= '".$frequency."'
							and date(created_on)<>date(now()) and sales_rep_id=A.sales_rep_id ) and date(date_of_visit)=date(now()) and bit_plan_id!=0 and is_edit='edit'
		...
		$sql2 = "Select  sum((Case When A.unplanned_count IS NULL THEN 0  Else A.unplanned_count end)) as unplanned_count ,((sum(Case When A.unplanned_count IS NULL THEN 0  Else A.unplanned_count end))+sum((Case When actual_count IS NULL THEN 0 Else actual_count end ))) as total_visit ,Sum(Case When F.check_in_time IS NULL OR  F.working_status='Absent' THEN 1 Else 0  end) as absent,Sum(Case When F.check_in_time IS NOT  NULL and F.working_status='Present' THEN 1 Else 0  end) as present,sum(Case WHEN O.od_units IS NULL Then 0 Else O.od_units end) as od_units,'' as od_value ,sum((Case When A.planned_count IS NULL THEN 0  Else A.planned_count end)) as planned_count,sum(actual_count) as actual_count
		from (
		SELECT A.sales_rep_name,A.id,B.planned_count,C.unplanned_count,C.sales_rep_id,C.zone_id From
		(SELECT * from sales_rep_master Where status='Approved' and sr_type='Sales Representative')A
		Left join
	   ...
	   $data2=$this->db->query($sql2)->result();
	    $to_email = "rishit.sanghvi@eatanytime.in, sales@eatanytime.co.in, manorma@eatanytime.in, raveer66@gmail.com, girish.rai@eatanytime.in, prasad.bhisale@pecanreams.com";
		/*$to_email = "ashwini.patil@pecanreams.com, dhaval.maru@pecanreams.com";
		$bcc = "sangeeta.yadav@pecanreams.com";*/
							...
							<td class="innerpadding " style="padding: 30px 30px 30px 30px;" >
							   <table class="table upper_table" style="width:765px;border-collapse: collapse;margin: 0 auto;" >
								...
								if(count($data)>0)
								{
									...
									$tbody.=(($data[0]->od_units==NULL || $data[0]->od_units=='') ?'0':$data[0]->od_units);
								}
								...
								if(count($data2)>0)
								{
									...
									$tbody.= (($data[0]->od_value==NULL || $data[0]->od_value=='') ?'0':$data[0]->od_value);
								}
								$tbody .='</span></td>
	...
	public function get_promoter_byemail($value='')
	{
		...
		$sql = "Select  A.*,E.p_call,D.actual_count,Case When (F.check_in_time IS NULL OR F.working_status='Absent') THEN 'Absent' Else 'Present' end as emp_status
			...
			Left join
			(SELECT count(*) as planned_count,sales_rep_id from merchandiser_beat_plan A Where date(created_on)<>date(now()) and frequency='$frequency' GROUP By sales_rep_id) B On A.id=B.sales_rep_id
			Left join
			(
				SELECT  sum(unplanned_count) as unplanned_count,sales_rep_id,zone_id from 
				(SELECT count(*) as unplanned_count,sales_rep_id,zone_id from merchandiser_beat_plan A Where store_id 
				In (Select store_id from merchandiser_detailed_beat_plan Where frequency='$frequency'  and sales_rep_id=A.sales_rep_id and is_edit='edit' and bit_plan_id!=0) and date(created_on)=date(now())
				GROUP By sales_rep_id,zone_id
				UNION
				SELECT count(*) as unplanned_count,sales_rep_id,zone_id from merchandiser_detailed_beat_plan B  Where bit_plan_id=0 and date(date_of_visit)=date(now()) and bit_plan_id=0 GROUP By sales_rep_id,zone_id
				Union
				Select count(*) as unplanned_count,sales_rep_id,zone_id from sales_rep_location Where followup_date IS NOT NULL and date(date_of_visit)=date(now()) GROUP By sales_rep_id,zone_id) A GROUP BY sales_rep_id,zone_id
			) C on A.id=C.sales_rep_id
			) A
			...
			LEFT JOIN ( 
				SELECT count(*) as actual_count ,sales_rep_id from ( 
							SELECT store_id,sales_rep_id,date_of_visit from merchandiser_detailed_beat_plan A 
							Where frequency= '".$frequency."' and store_id IN (Select DISTINCT store_id from merchandiser_beat_plan Where frequency= '".$frequency."'
							and date(created_on)<>date(now()) and sales_rep_id=A.sales_rep_id ) and date(date_of_visit)=date(now()) and bit_plan_id!=0 and is_edit='edit'
				) B GROUP BY sales_rep_id 
			) D 
			on D.sales_rep_id=A.id
	   ...
	   $tbody ='';
		$to_email = "rishit.sanghvi@eatanytime.in, dhaval.maru@pecanreams.com, swapnil.darekar@eatanytime.in, mukesh@eatanytime.in, sulochana@eatanytime.co.in, prasad.bhisale@pecanreams.com";
		/*$to_email = "ashwini.patil@pecanreams.com";
		$bcc = "sangeeta.yadav@pecanreams.com";*/
	 //  $bcc = "mis@eatanytime.co.in, ashwini.patil@pecanreams.com, sangeeta.yadav@pecanreams.com ";*/
	    $bcc = "ashwini.patil@pecanreams.com, sangeeta.yadav@pecanreams.com";
