...
function get_sales_rep_total_visits_email(){
    $sql = "SELECT count(id) as unplan from merchandiser_detailed_beat_plan Where date(date_of_visit)=date(now()) UNION SELECT count(id) as unplan from merchandiser_detailed_beat_plan Where date(date_of_visit)=date(now()) and  is_edit='edit' ;";
    $query=$this->db->query($sql);
    return $query->result();
}
...
function get_sales_rep1_email(){
    $sql = "select count(distinct(sales_rep_id)) as total_merchandiser from merchandiser_detailed_beat_plan";
    $query=$this->db->query($sql);
    return $query->result();
}
function get_sales_rep2_email(){
    $sql = "select count(distinct(sales_rep_id)) as present_merchandiser from merchandiser_detailed_beat_plan Where date(date_of_visit)=date(now())";
    $query=$this->db->query($sql);
    return $query->result();
}
