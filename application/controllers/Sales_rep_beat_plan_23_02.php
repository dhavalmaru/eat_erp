...
//$bcc = "dhaval.maru@pecanreams.com,swapnil.darekar@eatanytime.in,ashwini.patil@pecanreams.com,sangeeta.yadav@pecanreams.com";
	...
	public function get_promoter_byemail($value='')
	{
		...
		$sql = "Select  A.*,E.p_call,D.actual_count,Case When (F.check_in_time IS NULL OR F.working_status='Absent') THEN 'Absent' Else 'Present' end as emp_status
			...
			Left join
			(SELECT sum(Case When created_by=sales_rep_id and (created_on)=now() Then 0 Else 1  end
) as planned_count,sales_rep_id from merchandiser_beat_plan A Where frequency='$frequency' GROUP By sales_rep_id) B On A.id=B.sales_rep_id
			Left join
			(
				SELECT  sum(unplanned_count) as unplanned_count,sales_rep_id,zone_id from 
				(SELECT count(*) as unplanned_count,sales_rep_id,zone_id from merchandiser_beat_plan A Where store_id 
				In (Select store_id from merchandiser_detailed_beat_plan Where frequency='$frequency'  and sales_rep_id=A.sales_rep_id and is_edit='edit' and bit_plan_id!=0) and date(created_on)=date(now()) and created_by=sales_rep_id
				GROUP By sales_rep_id,zone_id
				UNION
			...
			LEFT JOIN ( 
				SELECT  count(*) as actual_count ,sales_rep_id from ( 
							...
							Where frequency= '$frequency' and store_id IN (SELECT Distinct Case When created_by=sales_rep_id 
			and (created_on)=now() Then Null Else store_id  end as store_id from merchandiser_beat_plan Where frequency='$frequency' and sales_rep_id=A.sales_rep_id) and bit_plan_id!=0 and is_edit='edit' and date(date_of_visit)=date(now())
				) B GROUP BY sales_rep_id 
			) D 
			on D.sales_rep_id=A.id
			Left Join 
