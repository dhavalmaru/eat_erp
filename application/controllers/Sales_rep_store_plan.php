...
class Sales_rep_store_plan extends CI_Controller{
    ...
    public function checkstatus($frequency='',$temp_date=''){
        ...
        if(count($result)>0) {
            ...
            $data['merchendizer']=$this->Sales_location_model->get_merchendiser_data('Approved','',$frequency,$temp_date);
            ...
            //$data['merchendizer_store_plan']=$this->Sales_location_model->get_merchendiser_detail('Approved','',$frequency,$temp_date)    ;
                   ...
                   if($follow_type!='')
                    {
                        if($follow_type=='gt_followup')
                        {
                            ...
                            {
                                $result = $this->db->query("SELECT id as stock_id ,sales_rep_loc_id, case When orange_bar IS NOT NULL and orange_bar!=0 Then CONCAT(orange_bar,'_Bar') end as orange_bar,
                        ...
                        else
                        {
                            ...
                            if($sales_loc_id!=NULL)
                            {
                                ...
                                Case When type='Bar' Then CONCAT(qty,'_Bar') ELSE CONCAT(qty,'_Box') end as qty,item_id,type
                                from
        ...
        if(count($result)>0) {
            if($result[0]->r_view == 1 || $result[0]->r_edit == 1) {
                ...
                if($get_channel_type=='GT'){
                    ...
                    {
                        $result = $this->Sales_location_model->get_data('Approved',$id,$frequency);
                    }*/
                    ...
                    else
                    {
                        $result = $this->db->query("SELECT id as stock_id ,sales_rep_loc_id,
                            ...
                            case When berry_blast_box IS NOT NULL and berry_blast_box!=0 Then CONCAT(berry_blast_box,'_Box')end as berry_blast_box,
                            case When chyawanprash_bar IS NOT NULL and chyawanprash_bar!=0 Then CONCAT(chyawanprash_bar,'_Bar') end as chyawanprash_bar,
                            case When chyawanprash_box IS NOT NULL and chyawanprash_box!=0 Then CONCAT(chyawanprash_box,'_Box') end as chyawanprash_box,chocolate_cookies_box,cranberry_orange_box,dark_chocolate_cookies_box,fig_raisins_box,papaya_pineapple_box,variety_box,cranberry_cookies_box,sales_rep_loc_id from  sales_rep_distributor_opening_stock Where sales_rep_loc_id=$sales_rep_loc_id")->result_array();
                        $this->db->last_query();
                        $data['stock_detail']=$result[0];
                    }
                    ...
                    {
                        $result=$this->Sales_location_model->get_merchendiser_data('Approved',$id,$frequency);
                    }
        ...
        if(count($result)>0) {
            if($result[0]->r_view == 1 || $result[0]->r_edit == 1) {
                ...
                { 
                    if($visit_detail['mid']!='')
                    {
                        echo 'mid'.$visit_id=$visit_detail['mid'];
                    }
        ...
        if($mid!='')
        {
              ...
              $detailed_id = $result[0]['id'];
                ...
                /*if (strpos($frequency, 'Every') !== false) {
                     ...
                     $where_array = array("store_id"=>$prev_store_id,
                                     ...
                                     "location_id"=>$prev_location_id,   
                                     "sales_rep_id"=>$prev_sales_rep_id,
                                     "frequency"=>'Alternate '.$explode_frequency[1],
                                     "modified_on" => $now,
                                     "modified_by" => $curusr
                                    );
                     $this->db->where($where_array)->update('merchandiser_beat_plan',$bit_plan_data);  
                ...
                if (strpos($frequency, 'Alternate') !== false) {
                     ...
                     $where_array = array("store_id"=>$prev_store_id,
                                     ...
                                     "location_id"=>$prev_location_id,   
                                     "sales_rep_id"=>$prev_sales_rep_id,
                                     "frequency"=>'Every '.$explode_frequency[1],
                                     "modified_on" => $now,
                                     "modified_by" => $curusr
                                    );
                     $this->db->where($where_array)->update('merchandiser_beat_plan',$bit_plan_data);  
                ...
                $update_details = array("store_id"=>$visit_store_id,
                                        'modified_on' => $now);
          ...
          { 
              ...
              $retailer_id = $visit_detail['store_id'];
                ...
                //removed 'followup_date'=>$followup_date
        ...
        else
        {
            ...
            else
            {
                ...
                {   
                    ...
                    /*echo 'sales_rep_loc_id'.$visit_detail['sales_rep_loc_id'];*/
                   ...
                   if($visit_detail['beat_plan_id']!='')
                    {
                        ...
                        if(count($detailed_result)>0)
                        {
                             ...
                             /*$detailed_insert_id = $merchendiser_beat_plan_id;*/
                             $this->db->where($where);
                             $this->db->update('sales_rep_detailed_beat_plan',$data);
