...
class Sales_rep_beat_plan extends CI_Controller{
	...
	public function get_sales_rep_email(){
        ...
        $data1 = $this->sr_beat_plan_model->get_sales_rep1_email();
        $data2 = $this->sr_beat_plan_model->get_sales_rep2_email();
        $orders= $this->sr_beat_plan_model->get_sales_rep_order_email();
        $total_visits = $this->sr_beat_plan_model->get_sales_rep_total_visits_email();
		$tbody ='';
						...
						<table bgcolor="#ffffff" class="content" style="max-width: 600px;" align="center" cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
						  ...
						  <tr>
							<td class="innerpadding " style="padding: 30px 30px 30px 30px;" >
							   <table class="table upper_table" style="width:765px;border-collapse: collapse;" >
									<tr>
										...
										if(count($total_visits)>=0)
										{
											if(isset($total_visits[1]->unplan) and ($total_visits[2]->unplan)){
								...
								if(count($orders)>0){  
								$tbody .=(isset($orders[0]->od_value)?$orders[0]->od_value:0);	
								}
								$tbody .='</span></td>
           ...
           <table  class="body_table" style="border-collapse: collapse;width:100%">
			<thead>
				<tr style=" background-color: #f3f3f3 ;border-bottom: 1px solid #ddd;font-weight: bold;">
				  <th style="border-right: 1px solid #ddd;padding: 3px;text-align: left;">Sr No</th>
				  <th style="border-right: 1px solid #ddd;padding: 3px;text-align: left;">Name Of Employee</th>
				  <th style="border-right: 1px solid #ddd;padding: 3px;text-align: left;">Beat Name</th>
				  <th style="border-right: 1px solid #ddd;padding: 3px;text-align: left;">Deviation from Route Plan</th>
				  <th style="border-right: 1px solid #ddd;padding: 3px;text-align: left;">Productive Calls</th>
				  <th style="border-right: 1px solid #ddd;padding: 3px;text-align: left;">% Productivity</th>
          ...
          '; 
				...
				if(count($data)>0) {
					for($i=0; $i<count($data); $i++){ 
								...
								$tbody.= ' <tr>
									<td style="padding: 3px;text-align: left;">'.($i+1).'</td>
									<td style="padding: 3px;text-align: left;">'.$data[$i]->sales_rep_name.'</td>
									<td style="padding: 3px;text-align: left;">'.$data[$i]->frequency.'</td>
									<td style="padding: 3px;text-align: left;">'.round($deviation,2).'%</td>
									<td style="padding: 3px;text-align: left;">'.(isset($data[$i]->p_call)?$data[$i]->p_call:0).'</td>
								
									<td style="padding: 3px;text-align: left;">'.$productivity. '</td>
									</tr>';
						  }
				}
