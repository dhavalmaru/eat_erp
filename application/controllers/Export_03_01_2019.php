...
class Export extends CI_Controller {
    ...
    public function set_report_criteria($rep_id){
        ...
        if($rep_id==16 || $rep_id==17){
            ...
            $data['merchandizer'] = $this->sales_rep_model->get_data_merchandizer('Approved');
    ...
    public function generate_report($rep_id) {
        if($rep_id==1) {
            ...
            if($this->input->post('excel_upload')!='' && $this->input->post('includes_zero')!=20)
            {
    ...
    public function upload_file() {
        ...
        if(count($batch_array)>0) {
            ...
            /*$telly_date = $this->input->post('telly_date');*/
            $telly_date = formatdate($this->input->post('telly_date'));
            $filename = $new_name;
