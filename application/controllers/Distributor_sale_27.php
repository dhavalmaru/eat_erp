            ...
            if(count($zone_id)!=0 || $distributor_id!='')
            {
               ...
               /* $sqlregion = "Select E.store_id, E.store_name from(
                ...
                if(count($region_result)>0)
                {
                    ...
                    {
                        if(!in_array($dist->store_name,$unique_store_name))
                        {
                            $unique_store_name[]=$dist->store_name;
                        }

                        if(!in_array($dist->location,$unique_location))
                        {
                            $unique_location[]=$dist->location;
                        }

                        if(!in_array($dist->zone,$unique_zone))
                        {
                            $unique_zone[]=$dist->zone;
                        }
                    }


                    $zn = 1;
                    foreach($unique_zone  as $zone)
                    {
                        $objPHPExcel->getActiveSheet()->setCellValue('B'.$zn,$zone);
                        $zn = $zn+1;
                    }

                    $rn = 1;
                    foreach($unique_store_name  as $store)
                    {
                        $objPHPExcel->getActiveSheet()->setCellValue('C'.$rn,$store);
                        $rn = $rn+1;
                    }
                    ...
                    {
                        $objPHPExcel->getActiveSheet()->setCellValue('D'.$ln,$location);
                        $ln = $ln+1;
                    }
