...
class Sales_rep_store_plan extends CI_Controller{
    ...
    public function checkstatus($frequency=''){
        ...
        if(count($result)>0) {
            ...
            $data['mt_followup']=$this->Sales_location_model->get_mtfollowup();
            $data['gt_followup']=$this->Sales_location_model->get_gtfollowup();
    ...
    public function add($id='',$get_frequency='',$get_channel_type='',$follow_type=''){
                            ...
                            if(count($result)>0)
                            {
                                for ($j=0; $j <count($result) ; $j++) {
                                   ...
                                   if ($result[$j]['item_id']==45) {
                                    ...
                                    else if($result[$j]['item_id']==16 && $result[$j]['type']=='Bar')
                                            $stock_detail['berry_blast']=$result[$j]['qty'];
            ...
            if($this->input->post('distributor_type')=="Old"){
                ...
                if($this->session->userdata('visit_detail')!=null)
                {
                   if($this->input->post('distributor_type')=='New')
                    {
                      redirect(base_url().'Sales_rep_store_plan/add_sales_rep_distributor');
                    }
                    else
                    {
    ...
    public function add_order(){
        ...
        if(count($result)>0) {
            if($result[0]->r_view == 1 || $result[0]->r_edit == 1) {
                ...
                if($this->session->userdata('sales_rep_stock_detail')!=null)
                {
                    $data['stock_detail']=$this->session->userdata('sales_rep_stock_detail');
                }
                else if($this->session->userdata('merchandiser_stock_details')!=null)
                {
                    $data['stock_detail']=$this->session->userdata('sales_rep_stock_detail');
                }
        ...
        if($mid!='')
        {
          ...
          { 
                ...
                else
                {
                    ...
                    $update_details = array("store_id"=>$store_id_d,
                                            'modified_on' => $now);
