...
function view_generate_distributor_balance_ledger_report() {
    ...
    if(count($data)>0) {
        ...
        for($i=0; $i<count($data); $i++){
            ...
            if((($openbal+$data[$i]->debit_amount)-$data[$i]->credit_amount)<20)
            {
                $datatable=$datatable.'<tr>';
                $datatable=$datatable. "<td>".($j+1)."</td>";
                $datatable=$datatable. "<td>".$data[$i]->distributor_name."</td>";
                $datatable=$datatable. "<td> 0</td>";
                $datatable=$datatable. "<td>".$data[$i]->debit_amount."</td>";
                $datatable=$datatable. "<td>".$data[$i]->credit_amount."</td>";
                $datatable=$datatable. "<td>".round((($openbal+$data[$i]->debit_amount)-$data[$i]->credit_amount),2)."</td>";
                $datatable=$datatable. "<tr>";  
                $j = $j+1;
            }
