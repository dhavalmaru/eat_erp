...
class Purchase_order extends CI_Controller{
    ...
    public function get_purchase_order_items(){
        ...
        if(count($result)>0) {
            ...
            for($i=0; $i<count($result); $i++){
                ...
                $data['qty'][$i] = (($result[$i]->qty==null)?0:$result[$i]->qty);
                $data['hsn_code'][$i] = (($result[$i]->hsn_code==null)?'':$result[$i]->hsn_code);
                $data['rate'][$i] = (($result[$i]->rate==null)?0:$result[$i]->rate);
                $data['tax_per'][$i] = (($result[$i]->tax_per==null)?0:$result[$i]->tax_per);
                $data['amount'][$i] = (($result[$i]->amount==null)?0:$result[$i]->amount);
                $data['cgst_amt'][$i] = (($result[$i]->cgst_amt==null)?0:$result[$i]->cgst_amt);
                $data['sgst_amt'][$i] = (($result[$i]->sgst_amt==null)?0:$result[$i]->sgst_amt);
                $data['igst_amt'][$i] = (($result[$i]->igst_amt==null)?0:$result[$i]->igst_amt);
                $data['tax_amt'][$i] = (($result[$i]->tax_amt==null)?0:$result[$i]->tax_amt);
                $data['total_amt'][$i] = (($result[$i]->total_amt==null)?$result[$i]->amount:$result[$i]->total_amt);
            }
        }
    ...
    public function update($id){
        ...
        // redirect(base_url().'index.php/purchase_order');
		   echo '<script>window.open("'.base_url().'index.php/purchase_order", "_parent")</script>';
    }
