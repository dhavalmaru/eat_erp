...
function get_data2($status='',$id='', $zone_id='', $area_id='', $location_id=''){
    ...
    $sales_rep_id=$this->session->userdata('sales_rep_id');
	 $sql = "select * from (SELECT  Distinct concat('d_',A.id) as id, A.sales_rep_id, A.distributor_name, 
            ...
            where A.status='approved' and A.type_id=3  and A.class='normal' and A.distributor_name<>'' ".$cond." 
			group by distributor_name,zone_id,area_id,location_id
        Union All
        select concat('s_',id) as id, B.sales_rep_id, B.distributor_name, 
            ...
            B.modified_by, B.modified_on 
                from sales_rep_distributors B where B.distributor_name<>'' ".$cond." 
         group by distributor_name,zone_id,area_id,location_id order by distributor_name asc) A  
              ";
    $query=$this->db->query($sql);
    return $query->result();
}
