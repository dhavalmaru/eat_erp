...
function get_distributor_out_details($from_date, $to_date) {
    $sql = "select L.*, J.item_name,J.quantity from
            ...
            (select *,MONTHNAME(STR_TO_DATE(month(date_of_processing),'%m')) as dmonth from distributor_out where status='Approved' and date_of_processing>='$from_date' and date_of_processing<='$to_date' and distributor_id!='189') A 
            left join 
            ...
            union all 
            select m.id, 'Box' as type, m.box_name as item_name, sum(p.qty) as quantity from box_master m left join box_product p on m.id=p.box_id where m.status='Approved' group by box_id) J 
...
function generate_sale_invoice_report() {
    ...
    if(count($data)>0) {
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            $cst=($data[$i]->cst/100)*($data[$i]->item_amount);
            $objPHPExcel->getActiveSheet()->setCellValue('J'.$row, $cst);
            $cstamt=$cst+$data[$i]->item_amount;
            $objPHPExcel->getActiveSheet()->setCellValue('K'.$row, round($cstamt),2);
            $objPHPExcel->getActiveSheet()->setCellValue('L'.$row, $data[$i]->amount);
            $objPHPExcel->getActiveSheet()->setCellValue('M'.$row, $data[$i]->cst_amount);
            $objPHPExcel->getActiveSheet()->setCellValue('N'.$row, $data[$i]->final_amount);
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('R'.$row, $data[$i]->location);
            $objPHPExcel->getActiveSheet()->setCellValue('S'.$row, $data[$i]->sales_rep_name);
            $objPHPExcel->getActiveSheet()->setCellValue('T'.$row, $data[$i]->due_date);
            $objPHPExcel->getActiveSheet()->setCellValue('U'.$row, $data[$i]->order_no);
            $objPHPExcel->getActiveSheet()->setCellValue('V'.$row, $data[$i]->order_date);
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('W'.$row, $barquantity);
            $objPHPExcel->getActiveSheet()->setCellValue('X'.$row, $data[$i]->remarks);
            $objPHPExcel->getActiveSheet()->setCellValue('Y'.$row, $data[$i]->dmonth);
            $row=$row+1;
        }
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1'.':Y'.$row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
...
function generate_distributor_ledger_report() {
    ...
    (select DATE_FORMAT(date_of_processing,'%d/%m/%Y') as ref_date, if(invoice_no IS NULL,'Sales Exchange',invoice_no) as reference, final_amount as debit_amount, null as credit_amount ,date_of_processing as rdate
...
function generate_payment_receivable_report() {
    ...
    //         order by K.id";
    $sql = "select DD.distributor_id, DD.distributor_name, DD.total_pending_amount, EE.invoice_date, EE.invoice_no, EE.invoice_amount, EE.voucher_no,DD.location from 
    ...
    if(count($data)>0) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+4].$row, "Invoice No");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+5].$row, "Amount");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+6].$row, "Voucher No");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+7].$row, "Location");
        ...
        for($i=0; $i<count($data); $i++){
            ...
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+4].$row, $data[$i]->invoice_no);
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+5].$row, $data[$i]->invoice_amount);
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+6].$row, $data[$i]->voucher_no);
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+7].$row, $data[$i]->location);
        }
...
function get_sales_rep_order_data($from_date, $to_date){
    $sql = "select C.date_of_processing, C.distributor_id, C.product_id, sum(C.qty) as qty from 
            ...
            (select * from sales_rep_orders where status = 'Active' and date_of_processing >= '$from_date' and date_of_processing <= '$to_date') A 
            left join 
...
function generate_promoter_stock_report_cron($from_date,$to_date) {
    ...
    $sql = "select I.* from 
                (select G.*, DATE_FORMAT(H.created_on,'%H:%i:%s') as out_time, DATE_FORMAT(TIMEDIFF(DATE_FORMAT(H.created_on,'%H:%i:%s'),in_time),'%H:%i:%s') as timediff from 
                (select E.*, F.orange, F.mint, F.butterscotch, F.chocopeanut, F.bambaiyachaat, F.mangoginger from 
                (select C.*, D.sales_rep_name from 
                (select A.*, DATE_FORMAT(A.created_on,'%H:%i:%s') as in_time, B.store_name from 
                (select DATE_FORMAT(date_of_visit,'%d/%m/%Y') as date_of_visit, created_on, distributor_id, sales_rep_id, id,id as p_id from promoter_location where status = 'Approved' and 
