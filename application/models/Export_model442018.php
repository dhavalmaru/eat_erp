...
function get_distributor_out_details($from_date, $to_date) {
    ...
    $sql = "select L.*, M.area from 
            ...
            (select E.*, F.distributor_name, F.sell_out, F.type_id, F.location_id, F.city as distributor_city, F.area_id, F.class from 
...
function get_sample_expired_details($from_date, $to_date) {
    ...
    $sql = "select AA.*, WEEK(date_of_processing,1)-WEEK(STR_TO_DATE(concat(YEAR(date_of_processing),'-',MONTH(date_of_processing),'-',1),'%Y-%m-%d'),1)+1 as dweek from 
            ...
            (select A.*, A.id as sampleid, C.depot_name, D.distributor_name, D.sell_out, D.type_id, D.location_id, D.city as distributor_city, D.class, 
...
function generate_sale_invoice_sku_report($sales,$ssallocation,$salesreturn,$sample,$status_type='') {
    ...
    if($ssallocation!="") {
        ...
        for($k=0; $k<count($data); $k++){
            ...
            if($status=='Cancelled') {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('W'.$row, $data[$k]->area);
    ...
    if($sample!="") {
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            if($status=='Cancelled') {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('S'.$row, $data[$i]->dname);
