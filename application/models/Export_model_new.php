...
function get_distributor_out_details($from_date, $to_date) {
    $sql = "select A.*, D.depot_name, D.state as Depot_state, D.state_code as depot_state_code, 
                    L.distributor_type, H.sales_rep_name, K.location, M.area, O.zone as dist_zone from 
...
function get_distributor_out_sku_details($from_date, $to_date, $status='', $date_of_processing='', $date_of_accounting='') {
    ...
    $sql = "select * from 
            (select AA.*, WEEK(invoice_date,1)-WEEK(STR_TO_DATE(concat(YEAR(invoice_date),'-',MONTH(invoice_date),'-',1),'%Y-%m-%d'),1)+1 as dweek from 
            (select A.id as saleid, A.id, A.distributor_id, A.invoice_date, A.invoice_no, A.state, A.state_code, A.con_state_code, A.con_state, A.con_gst_number, A.due_date, A.order_no, A.order_date, A.shipping_address, A.round_off_amount, A.status, A.remarks, C.depot_name, C.state as depot_state, C.state_code as depot_state_code, D.distributor_name, D.sell_out, D.type_id, D.location_id, D.area_id, D.zone_id, D.city as distributor_city, D.class, D.state as dist_state, D.state_code as dist_state_code, D.gst_number as dist_gst_no, D.prefix, E.distributor_type, F.location, G.area, H.zone, J.sales_rep_name, K.sales_rep_name as salesrepname, L.sales_rep_name as salesrepname1 
            from distributor_out A 
            left join depot_master C on(A.depot_id=C.id) 
            left join distributor_master D on(A.distributor_id=D.id) 
            left join distributor_type_master E on(D.type_id=E.id) 
            left join location_master F on(D.location_id=F.id) 
            left join area_master G on(D.area_id=G.id) 
            left join zone_master H on(D.zone_id=H.id) 
			left join sr_mapping I on(D.area_id=I.area_id and D.type_id=I.type_id and D.zone_id=I.zone_id) 
			left join sales_rep_master J on(I.reporting_manager_id=J.id) 
            left join sales_rep_master K on(I.sales_rep_id1=K.id) 
            left join sales_rep_master L on(I.sales_rep_id2=L.id) 
    ...
    if($sales!="") {
        ...
        if(count($data)>0) {
            for($i=0; $i<count($data); $i++) {
                ...
                $sql = "select * from 
                        (select AA.*, BB.item_name, BB.quantity, BB.short_name, CC.category_name from 
                        (select B.type, B.item_id, B.qty, B.sell_rate, B.grams, B.rate, B.amount as item_amount, B.cgst_amt, B.sgst_amt, B.igst_amt, B.tax_amt, B.total_amt from distributor_out_items B where B.distributor_out_id='$id') AA 
                        left join 
                        (select id, 'Bar' as type, short_name as item_name, null as quantity, short_name, category_id from product_master where status='Approved' 
                        union all 
                        select m.id, 'Box' as type, m.box_name as item_name, sum(p.qty) as quantity, m.short_name, m.category_id from box_master m left join box_product p on m.id=p.box_id where m.status='Approved' group by m.id) BB 
                        on (AA.item_id=BB.id and AA.type=BB.type) 
                        left join category_master CC on (BB.category_id=CC.id)) DD";
                $query=$this->db->query($sql);
                $data2=$query->result();
                ...
                if(count($data2)>0) {
                    for($j=0; $j<count($data2); $j++) {
                        ...
                        $objPHPExcel->getActiveSheet()->setCellValue('G'.$row, "SALES");
                        $objPHPExcel->getActiveSheet()->setCellValue('H'.$row, $data[$i]->invoice_no);
                        $objPHPExcel->getActiveSheet()->setCellValue('I'.$row, $data2[$j]->item_name);
                        $objPHPExcel->getActiveSheet()->setCellValue('J'.$row, $data2[$j]->short_name);
                        $objPHPExcel->getActiveSheet()->setCellValue('K'.$row, $data2[$j]->type);
                        $objPHPExcel->getActiveSheet()->setCellValue('L'.$row, $data2[$j]->category_name);
                        $objPHPExcel->getActiveSheet()->setCellValue('M'.$row, $data2[$j]->qty);
                        $quantity=$data2[$j]->quantity;
        				$distributor_name=$data[$i]->distributor_name;
                        $barquantity=0;
                        if($data2[$j]->quantity==null) {
                            $barquantity=1*$data2[$j]->qty;
                        } else {
                            $barquantity=$data2[$j]->quantity*$data2[$j]->qty;
                        }

                        $objPHPExcel->getActiveSheet()->setCellValue('N'.$row, $barquantity);
                        $objPHPExcel->getActiveSheet()->setCellValue('O'.$row, $data2[$j]->rate);
                        $objPHPExcel->getActiveSheet()->setCellValue('P'.$row, $data2[$j]->sell_rate);
                        ...
                        if($status=='Cancelled') {
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValue('Q'.$row, $data2[$j]->item_amount);

                            // $tax=($data2[$j]->tax_per/100)*($data2[$j]->item_amount);
                            // $objPHPExcel->getActiveSheet()->setCellValue('Q'.$row, $tax);
                            // $cstamt=$tax+$data2[$j]->item_amount;
                            // $objPHPExcel->getActiveSheet()->setCellValue('S'.$row, $cstamt);

                            $cstamt=$data2[$j]->total_amt;

                            if($data2[$j]->tax_amt==null || $data2[$j]->tax_amt==''){
                                $tax_amt = 0;
                                $cgst_amt = 0;
                                $sgst_amt = 0;
                                $igst_amt = 0;
                            } else {
                                $tax_amt = $data2[$j]->tax_amt;
                                if($data2[$j]->igst_amt==null || $data2[$j]->igst_amt=='' || $data2[$j]->igst_amt==0){
                                    $cgst_amt = round($tax_amt/2,2);
                                    $sgst_amt = $tax_amt - $cgst_amt;
                                    $igst_amt = 0;
                                } else {
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValue('W'.$row, $data2[$j]->total_amt);
