...
class Box_to_bar_model Extends CI_Model{
...
function get_access(){
    ...
    $query=$this->db->query("SELECT * FROM user_role_options WHERE section = 'Box_to_bar' AND role_id='$role_id' AND (r_insert = 1 OR r_view = 1 OR r_edit=1 OR r_approvals = 1 OR r_export = 1)");
    return $query->result();
}
...
function get_box_to_bar_qty($id){
    $sql = "select A.*, B.box_name from 
            (select * from box_to_bar_qty where box_to_bar_id = '$id') A 
            left join 
...
function save_data($id=''){
    ...
    if($id==''){
        ...
        $this->db->insert('box_to_bar',$data);
        $id=$this->db->insert_id();
        $action='Bar to box Entry Created.';
    } else {
        $this->db->where('id', $id);
        $this->db->update('box_to_bar',$data);
        $action='Bar to box Entry Modified.';
    }

    $this->db->where('box_to_bar_id', $id);
    $this->db->delete('box_to_bar_qty');
    ...
    $logarray['module_name']='Box_to_bar';
    $logarray['cnt_name']='Box_to_bar';
