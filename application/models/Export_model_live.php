...
function view_distributor_ledger_report() {
    ...
    $sql = "select DATE_FORMAT('$from_date','%d/%m/%Y') as ref_date, 'Opening Balance' as reference, 
            ...
            debit_amount, credit_amount from 

            (select sum(debit_amount) as debit_amount, sum(credit_amount) as credit_amount from 
            ...
            select DATE_FORMAT(date_of_transaction,'%d/%m/%Y') as ref_date, ref_no as reference, 
                ...
                date_of_transaction<'$from_date') A) B";
...
public function send_mt_stock_tracker() {
    ...
    if(count($result)>0){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $sql = "select * from merchandiser_stock where dist_id = '$store_id' and location_id = '$location_id' 
            ...
            if($bl_flag==false){
                ...
                $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+4].$row, 'C');
        ...
        $filename='MT_Stock_Tracker_'.date("d-m-Y").'.xlsx';
    ...
    // $from_email = 'cs@eatanytime.co.in';
    // $from_email_sender = 'Wholesome Habits Pvt Ltd';
    // $subject = 'MT_Stock_Tracker_'.date("d-m-Y");
