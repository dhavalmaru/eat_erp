...
public function get_mtfollowup($temp_date='')
{
    if($temp_date!='')
    {
        $date = date('Y-m');
  ...
  $sql = "SELECT A.*, D.is_edit, D.is_visited  from 
            ...
            (SELECT * from merchandiser_stock Where date(followup_date)=$date)A
            left JOIN
...
public function get_gtfollowup($id='',$temp_date='')
{
   ...
   $sql = "SELECT A.*,B.*,D.is_edit,D.is_visited from 
            (SELECT A.* from 
            (SELECT id ,id as sales_rep_loc_id  ,zone_id,location_id,area_id,distributor_id as store_id,followup_date,distributor_name,distributor_type,distributor_status
                from  sales_rep_location Where date(followup_date)=$date ) A
            left JOIN
            (Select Distinct C.* FROM(
                            Select B.* from (
                                Select concat('d_',A.id) as id , A.distributor_name ,A.google_address,A.latitude,A.longitude,'' as gst_number,'' as margin,'' as doc_document,' ' as document_name FROM
...
function get_merchendiser_data($status='', $id='',$frequency=''){
    ...
    $sql = "select G.*,H.date_of_visit,H.dist_id,H.id as mid,B.location,'Old' as distributor_type from (select E.*,F.sales_rep_name from(select C.*, D.google_address,D.latitude,D.longitude from (select A.*,B.store_name from 
      (".$table_name.$cond.' and sales_rep_id='.$sales_rep_id.") A 
      left join 
      (SELECT * FROM relationship_master where type_id ='4' or type_id='7') B 
      on (A.store_id=B.id))C
