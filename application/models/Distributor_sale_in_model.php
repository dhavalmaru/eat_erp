...
class Distributor_sale_in_model Extends CI_Model{
...
function get_data($status='', $id=''){
    ...
    $sql = "select A.*, B.store_name from 
            (select * from distributor_sale_in".$cond.") A 
            left join 
            (select * from relationship_master) B 
            on (A.store_id=B.id) where A.status='Approved' order by A.modified_on desc";
    $query=$this->db->query($sql);
    return $query->result();
}

function get_distributor_sale_items($id){
    $sql = "select * from distributor_sale_in_items where distributor_sale_in_id = '$id'";
    $query=$this->db->query($sql);
    return $query->result();
}
...
function save_data($id=''){
    ...
    if($id==''){
        ...
        $this->db->insert('distributor_sale_in',$data);
        $id=$this->db->insert_id();
        $action='Distributor Sale Entry Created.';
    } else {
        $this->db->where('id', $id);
        $this->db->update('distributor_sale_in',$data);
        $action='Distributor Sale Entry Modified.';
    }

    $this->db->where('distributor_sale_in_id', $id);
    $this->db->delete('distributor_sale_in_items');
    ...
    for ($k=0; $k<count($type); $k++) {
        if(isset($type[$k]) and $type[$k]!="") {
            ...
            $this->db->insert('distributor_sale_in_items', $data);
        }
    }
...
// function get_store(){
    // $sql = "select * from relationship_master";
    // $query=$this->db->query($sql);
    // return $query->result();
// }

function get_distributor_master(){
...
function get_store($zone_id){
    ...
    $sql = "Select  E.* from(Select  A.*,D.store_name from 
