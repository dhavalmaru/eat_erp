...
function get_distributor_out_details($from_date, $to_date) {
    ...
    $sql = " select L.*, M.area from 
            ...
            (select E.*, F.distributor_name, F.sell_out, F.type_id, F.zone_id,F.location_id, F.city as distributor_city, F.area_id, F.class from 
            ...
            on (L.area_id=M.id)
            where L.class!='sample' or L.class is null 
...
function get_sample_expired_details($from_date, $to_date) {
    ...
    $sql = "select AA.*, WEEK(date_of_processing,1)-WEEK(STR_TO_DATE(concat(YEAR(date_of_processing),'-',MONTH(date_of_processing),'-',1),'%Y-%m-%d'),1)+1 as dweek from 
            ...
            (select A.*, A.id as sampleid, C.depot_name, D.distributor_name, D.sell_out, D.type_id,D.zone_id,D.area_id, D.location_id, D.city as distributor_city, D.class, 
...
function get_distributor_in_details($from_date, $to_date) {
    ...
    $sql = "select AA.*, WEEK(date_of_processing,1)-WEEK(STR_TO_DATE(concat(YEAR(date_of_processing),'-',MONTH(date_of_processing),'-',1),'%Y-%m-%d'),1)+1 as dweek from 
            ...
            (select A.*, A.id as srid, C.depot_name, D.distributor_name, D.sell_out, D.type_id, D.location_id, D.city as distributor_city, D.class, 
...
function generate_sale_invoice_report($invoicelevel, $invoicelevelsalesreturn, $invoicelevelsample) {
        ...
        if(count($data)>0) {
            ...
            for($i=0; $i<count($data); $i++) {
                ...
                if($status=='Cancelled') {
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('I'.$row, $data[$i]->tax_amount);

                    $round_off_amt=round($data[$i]->final_amount)-$data[$i]->final_amount;

                    $objPHPExcel->getActiveSheet()->setCellValue('J'.$row, $round_off_amt);

                    $objPHPExcel->getActiveSheet()->setCellValue('K'.$row, round($data[$i]->final_amount));

                    $objPHPExcel->getActiveSheet()->setCellValue('L'.$row, $data[$i]->depot_name);

                    $objPHPExcel->getActiveSheet()->setCellValue('M'.$row, $data[$i]->distributor_name);

                    $objPHPExcel->getActiveSheet()->setCellValue('N'.$row, $data[$i]->distributor_city);

                    $objPHPExcel->getActiveSheet()->setCellValue('O'.$row, $data[$i]->distributor_type);

                    $objPHPExcel->getActiveSheet()->setCellValue('P'.$row, $data[$i]->location);

                    $objPHPExcel->getActiveSheet()->setCellValue('Q'.$row, $data[$i]->area);

                    $objPHPExcel->getActiveSheet()->setCellValue('R'.$row, $data[$i]->sales_rep_name);

                    $objPHPExcel->getActiveSheet()->setCellValue('S'.$row, $data[$i]->due_date);

                    $objPHPExcel->getActiveSheet()->setCellValue('T'.$row, $data[$i]->order_no);

                    $objPHPExcel->getActiveSheet()->setCellValue('U'.$row, $data[$i]->order_date);

                }

                $objPHPExcel->getActiveSheet()->setCellValue('V'.$row, $data[$i]->remarks);

                $objPHPExcel->getActiveSheet()->setCellValue('W'.$row, $data[$i]->id);

                $objPHPExcel->getActiveSheet()->setCellValue('X'.$row, $status);
    ...
    if($invoicelevelsample!="") {
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            if($status=='Cancelled') {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('M'.$row, $data[$i]->dname);
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('T'.$row, $data[$i]->sample_type);
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('W'.$row, $data[$i]->sampleid);
    ...
    if($invoicelevelsalesreturn!="") {
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('W'.$row, $data[$i]->srid);
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A8'.':X'.$row)->applyFromArray(array(
...
function get_distributor_out_sku_details($from_date, $to_date, $status='') {
    ...
    $sql = "select * from 
            ...
            (select A.*,A.id as saleid, B.type, B.item_id, B.qty, B.sell_rate, B.grams, B.rate, B.amount as item_amount, 
                ...
                C.depot_name, D.distributor_name, D.sell_out, D.type_id, D.location_id,D.area_id,D.zone_id,D.city as distributor_city, D.class, 

                E.distributor_type, F.sales_rep_name, G.location, H.area ,K.zone,(Select sales_rep_name from sales_rep_master where id=J.sales_rep_id1) as salesrepname,(Select sales_rep_name from sales_rep_master where id=J.sales_rep_id2) as salesrepname1
            ...
            where A.invoice_date>='$from_date' and A.invoice_date<='$to_date' and (A.status='Approved' or A.status='InActive')) AA 
...
function get_distributor_sale_sku_details($from_date, $to_date) {
    ...
    $sql = "select AA.*, WEEK(date_of_processing,1)-WEEK(STR_TO_DATE(concat(YEAR(date_of_processing),'-',
            ...
            (select A.*,A.id as ssid, B.type, B.item_id, B.qty, B.sell_rate, B.grams, B.rate, B.amount as item_amount, 
                ...
                D.distributor_name, D.sell_out, D.type_id, D.location_id, D.city as distributor_city, D.area_id,D.zone_id,

                E.distributor_type, G.location, H.distributor_name as m_distributor_name, 

                H.distributor_location as m_distributor_location,

                H.distributor_type as m_distributor_type, 
				F.sales_rep_name,K.zone,
                I.area,(Select sales_rep_name from sales_rep_master where id=J.sales_rep_id1) as salesrepname,(Select sales_rep_name from sales_rep_master where id=J.sales_rep_id2) as salesrepname1,M.store_id,O.store_name, Q.distributor_type as d_type,P.zone as d_zone,S.sales_rep_name as reporting_manager,
				(Select sales_rep_name from sales_rep_master where id=R.sales_rep_id1) as sales1,(Select sales_rep_name from sales_rep_master where id=R.sales_rep_id2) as sales2
                ...
                left join distributor_type_master E on(D.type_id=E.id) 
				left join super_stockist_distributor H on(A.to_distributor_id = H.id) 
				...
				left join store_master M on(A.to_distributor_id = M.store_id) 
				...
				left join sr_mapping R on(M.store_id=R.area_id1 and M.type_id=R.type_id and M.zone_id=R.zone_id )
...
function generate_sale_invoice_sku_report($sales,$ssallocation,$salesreturn,$sample,$credit_debit='',$status_type='') {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('B4', $fromdate);

    $objPHPExcel->getActiveSheet()->setCellValue('E4', $todate);
    ...
    if($sales!="") {
        ...
        $data = $this->get_distributor_out_sku_details($from_date, $to_date, $status_type);
        ...
        if(count($data)>0) {
            ...
            for($i=0; $i<count($data); $i++) {
                ...
                if($bl_insert == true){
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('S'.$row, $round_off_amt);
    ...
    if($ssallocation!="") {
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            if($status=='Cancelled') {
                ...
                // $cstamt=$tax+$data[$i]->item_amount;
				  ...
				  $objPHPExcel->getActiveSheet()->setCellValue('R'.$row, '');
					$objPHPExcel->getActiveSheet()->setCellValue('S'.$row, '-'.$data[$i]->item_amount);
					...
					$objPHPExcel->getActiveSheet()->setCellValue('AE'.$row, $data[$i]->ssid);
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('AH'.$row, $data[$i]->ssid);
        ...
        for($k=0; $k<count($data); $k++){
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('E'.$row, $data[$k]->modified_on);
            ...
            if($status=='Cancelled') {
                ...
                // $cstamt=$tax+$data[$k]->item_amount;
				  ...
				  $objPHPExcel->getActiveSheet()->setCellValue('Q'.$row, '');
				  $objPHPExcel->getActiveSheet()->setCellValue('R'.$row, '');
				    $objPHPExcel->getActiveSheet()->setCellValue('S'.$row, $data[$k]->item_amount);
				      $objPHPExcel->getActiveSheet()->setCellValue('T'.$row, '');

                $objPHPExcel->getActiveSheet()->setCellValue('U'.$row, $data[$k]->m_distributor_name);

          

                $objPHPExcel->getActiveSheet()->setCellValue('V'.$row, $data[$k]->d_type);
                $objPHPExcel->getActiveSheet()->setCellValue('W'.$row, $data[$k]->d_zone);
                $objPHPExcel->getActiveSheet()->setCellValue('X'.$row, $data[$k]->store_name);
                $objPHPExcel->getActiveSheet()->setCellValue('Y'.$row, $data[$k]->distributor_city);
               

                $objPHPExcel->getActiveSheet()->setCellValue('Z'.$row, $data[$k]->m_distributor_location);

                $objPHPExcel->getActiveSheet()->setCellValue('AA'.$row, $data[$k]->reporting_manager);
                $objPHPExcel->getActiveSheet()->setCellValue('AB'.$row, $data[$k]->sales1);
                $objPHPExcel->getActiveSheet()->setCellValue('AC'.$row, $data[$k]->sales2);

                $objPHPExcel->getActiveSheet()->setCellValue('AD'.$row, '');

      

                $objPHPExcel->getActiveSheet()->setCellValue('AE'.$row, $data[$k]->ssid);
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('AG'.$row, $data[$k]->remarks);

            $objPHPExcel->getActiveSheet()->setCellValue('AH'.$row, $data[$k]->ssid);
    ...
    if($sample!="") {
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            if($status=='Cancelled') {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('AE'.$row, $data[$i]->sample_type);
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('AH'.$row, $data[$i]->sampleid);
    ...
    if($salesreturn!="") {
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('E'.$row, $data[$i]->dweek);
            $objPHPExcel->getActiveSheet()->setCellValue('F'.$row, $data[$i]->modified_on);
	 ...
	 if($credit_debit!="") {
        ...
        $include=$include.'credit_debit, ';

        $data = $this->get_credit_debit_sku_details($from_date, $to_date);
            ...
            for($i=0; $i<count($data); $i++) {
               ...
               // $quantity=$data[$i]->quantity;
                ...
                if($status=='Cancelled') {
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('P'.$row, $data[$i]->amount);
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A10:AD10')->getFont()->setBold(true);

    

    $objPHPExcel->getActiveSheet()->getStyle('A10'.':AD'.$row)->applyFromArray(array(
...
function get_credit_debit_sku_details($from_date, $to_date) {

    $sql = "
            select A.*, D.distributor_name, D.sell_out, D.type_id, D.location_id, D.city as distributor_city, D.class,D.area_id,D.zone_id,K.zone,E.distributor_type, F.sales_rep_name, G.location, H.area ,(Select sales_rep_name from sales_rep_master where id=J.sales_rep_id1) as salesrepname,(Select sales_rep_name from sales_rep_master where id=J.sales_rep_id2) as salesrepname1
            ...
            where (A.status='Approved' or A.status='InActive') and A.date_of_transaction>='$from_date' and A.date_of_transaction<='$to_date'";
...
function get_sample_expired_SKU_details($from_date, $to_date) {
    ...
    $sql = "select AA.*, WEEK(date_of_processing,1)-WEEK(STR_TO_DATE(concat(YEAR(date_of_processing),'-',MONTH(date_of_processing),'-',1),'%Y-%m-%d'),1)+1 as dweek, 
            ...
            (select A.*, A.id as sampleid, B.type, B.item_id, B.qty, B.sell_rate, B.grams, B.rate, B.amount as item_amount, 
                ...
                C.depot_name, D.distributor_name, D.sell_out, D.type_id, D.location_id, D.city as distributor_city, D.class,D.area_id,D.zone_id,K.zone,E.distributor_type, F.sales_rep_name, G.location, I.distributor_name as dname, H.area ,(Select sales_rep_name from sales_rep_master where id=J.sales_rep_id1) as salesrepname,(Select sales_rep_name from sales_rep_master where id=J.sales_rep_id2) as salesrepname1
...
function get_distributor_in_sku_details($from_date, $to_date) {
    ...
    $sql = "select AA.*, WEEK(date_of_processing,1)-WEEK(STR_TO_DATE(concat(YEAR(date_of_processing),'-',MONTH(date_of_processing),'-',1),'%Y-%m-%d'),1)+1 as dweek, 
            ...
            (select A.*, A.id as srid, B.type, B.item_id, B.qty, B.sell_rate, B.grams, B.rate, B.amount as item_amount, 
                ...
                C.depot_name, D.distributor_name,K.zone, D.sell_out, D.type_id, D.location_id,D.area_id,D.zone_id, D.city as distributor_city, D.class, 

                E.distributor_type, F.sales_rep_name, G.location, H.area ,(Select sales_rep_name from sales_rep_master where id=J.sales_rep_id1) as salesrepname,(Select sales_rep_name from sales_rep_master where id=J.sales_rep_id2) as salesrepname1
...
function get_product_stock_details($from_date, $to_date) {
    ...
    $sql = "select * from 
            ...
            (select QQ.depot_id, QQ.product_id, QQ.opening_qty, QQ.production_qty, QQ.depot_in_qty, QQ.depot_out_qty, QQ.sale_qty, QQ.sample_qty, QQ.expire_qty, QQ.sale_return_qty, RR.product_name as item_name, RR.grams as unit_weight from 
...
function generate_distributor_ledger_report() {
    ...
    $sql = "select DATE_FORMAT('$from_date','%d/%m/%Y') as ref_date, 'Opening Balance' as reference, 
            ...
            (select DATE_FORMAT(date_of_processing,'%d/%m/%Y') as ref_date, 
                ...
                final_amount as debit_amount, null as credit_amount, date_of_processing as rdate, remarks as remarks 
            ...
            (select DATE_FORMAT(A.date_of_deposit,'%d/%m/%Y') as ref_date, B.invoice_no as reference, null as debit_amount, 

                B.payment_amount as credit_amount, A.date_of_deposit as rdate, A.remarks as remarks from 
            ...
            select DATE_FORMAT(date_of_transaction,'%d/%m/%Y') as ref_date, transaction as reference, 
                    ...
                    case when transaction='Credit Note' then amount end as credit_amount, date_of_transaction as rdate, remarks as remarks 
    ...
    $sql = "select ref_date, reference,debit_amount,credit_amount,rdate,remarks from 
    ...
    if(count($data)>0) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+4].$row, 'Running Balance');
        ...
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+1].$row, $open_bal_data[0]->reference);

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+2].$row, $open_bal_data[0]->debit_amount);

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+3].$row, $open_bal_data[0]->credit_amount);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+4].$row, $running_balance);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A6:'.$col_name[$col+5].$row)->applyFromArray(array(
...
function view_distributor_ledger_report() {
    ...
    if(count($data)>0) {
        ...
        $datatable='<table id="printTable" border="1" cellpadding="3" style="border-collapse: collapse;">
