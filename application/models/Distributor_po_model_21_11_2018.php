...
function get_store_data($status='', $type_id='', $zone_id=''){
    ...
    if($type_id!=""){
        if($cond=="") {
            $cond=" where A.type_id='$type_id'";
        } else {
            $cond=$cond." and A.type_id='$type_id'";
        }
    }
...
function get_location_data($status='', $type_id='', $zone_id='', $store_id=''){
    ...
    $sql = "select distinct A.location_id, B.location from 
            (select A.* from store_master A ".$cond.") A 
            left join 
...
function set_delivery_status() {
    ...
    if($distributor_po_id!=""){
        ...
        if($delivery_status=="Cancelled"){
            $sql = "update distributor_po set delivery_status = '$delivery_status', cancelled_date = '".date('Y-m-d')."', 
            ...
            $sql = "update distributor_po set delivery_status = '$delivery_status', delivery_date = '".date('Y-m-d')."', 
                    status = '$status', modified_by = '$curusr', modified_on = '$now', 
                    remarks = concat(remarks, '$remarks'), delivery_remarks = '$delivery_remarks' 
                    where id in (".$distributor_po_id.")";
        }
...
function generate_po_delivery_report() {
    $sql = "select AA.id, AA.date_of_po, AA.po_expiry_date, AA.po_number, AA.status, AA.remarks, 
            ...
            where (A.status='Approved' or A.status='InActive')) AA 
    ...
    if(count($data)>0) {
        ...
        for($i=0; $i<count($data); $i++){
            ...
            } else if(strtoupper(trim($data[$i]->item_type))=='BOX' && strtoupper(trim($data[$i]->delivery_status))=='DELIVERED' && ($data[$i]->item_id=='26' || $data[$i]->item_id=='27')){
