...
function get_sales_rep_total_visits_email($frequency=''){
    ...
    $sql = "SELECT count(store_id) as unplan from sales_rep_detailed_beat_plan Where date(date_of_visit)=date(now()) and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)= date(now()))and frequency='".$frequency."' UNION SELECT count(store_id) as unplan from sales_rep_detailed_beat_plan Where date(date_of_visit)=date(now()) and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)<>date(now())) and frequency='".$frequency."'UNION SELECT count(store_id) as unplan from sales_rep_detailed_beat_plan Where date(date_of_visit)=date(now()) and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)<>date(now()))and is_edit='edit' and frequency='".$frequency."' ;";
    $query=$this->db->query($sql);
    return $query->result();
}
...
function get_sales_rep_email($frequency=''){
	 ...
	 $sql="
			...
			(
				...
				IN (Select DISTINCT store_id from sales_rep_beat_plan Where frequency='".$frequency."' and date(created_on)=date(now()) ) 
