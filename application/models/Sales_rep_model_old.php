...
function get_data_dist($status='', $id=''){
    ...
    if($id!=""){
        if($cond=="") {
            $cond=" where id='".$id."' and (sr_type is null or sr_type!='Promoter')";
        } else {
     ...
     if($ispermenant=='Yes')
        {
            ...
            $sql = "
            DELETE FROM admin_sales_rep_beat_plan WHERE sales_rep_id=$sales_rep_id and frequency='$frequency'";
            $result = $this->db->query($sql);   

            $sql = "
            Insert INTO admin_sales_rep_beat_plan (store_id,frequency,sequence,created_by,
            created_on,modified_by,modified_on,status,distributor_id,area_id,zone_id,location_id,sales_rep_id)Select store_id,frequency,sequence,created_by,created_on,$curusr,'$now',status,distributor_id,area_id,zone_id,location_id,sales_rep_id  from sales_rep_beat_plan Where sales_rep_id=$sales_rep_id and frequency='$frequency'";
            $result = $this->db->query($sql);     
        }
}
}
