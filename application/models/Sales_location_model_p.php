...
function get_data($status='', $id='',$frequency=''){
     ...
     $sql = "Select sequence from sales_rep_detailed_beat_plan Where  sales_rep_id=$sales_rep_id 
      ...
      else
      {
       ...
       $sql = "select G.*,H.date_of_visit,H.id as mid ,H.distributor_type,H.remarks,H.followup_date  from (select E.*,F.sales_rep_name from(select C.* from (select A.*,B.distributor_name ,B.distributor_name as store_name ,B.google_address,B.latitude,B.longitude,B.gst_number,B.margin,B.doc_document,B.document_name from 
            (".$table_name.") A 
            left join 
...
public function get_merchendiser_detail($status='', $id='',$frequency='')
{
    ...
    $sql = "select G.*,H.date_of_visit,H.dist_id,H.id as mid,H.location_id,B.location from (select E.*,F.sales_rep_name from(select C.*, D.google_address,D.latitude,D.longitude from (select A.*,B.store_name from 
...
public function get_mtfollowup($id='')
{   
    $cond = '';
    if($id!=''){
        $cond = $cond . " and id = '$id' ";
    ...
    $sql = "select A.*, D.is_edit, D.is_visited  from 
            ...
            (SELECT * from merchandiser_stock Where date(followup_date)=date(now())".$cond.")A
            left JOIN
...
public function get_gtfollowup($id='')
{
   $cond= '';
   if($id!='')
        $cond="Where id=".$id;
   ...
   $sql = "SELECT A.*,B.*,D.is_edit,D.is_visited from 
            (SELECT A.* from 
            (SELECT id  ,zone_id,location_id,area_id,distributor_id as store_id,followup_date,distributor_name,distributor_type,distributor_status
                from  sales_rep_location Where date(followup_date)=date(now()) ) A
            left JOIN
            (Select Distinct C.* FROM(
                            Select B.* from (
                                Select concat('d_',A.id) as id , A.distributor_name ,A.google_address,A.latitude,A.longitude,'' as gst_number,'' as margin,'' as doc_document,' ' as document_name FROM
