...
function get_sales_rep_total_visits_email($frequency=''){
	  $day = date('l');
        ...
        $sql = "select * from (SELECT (A.unplan+A.temp_count) as unplan, sales_rep_id from (SELECT count(store_id) as unplan,(Select count(id) as tem_visit_count from sales_rep_detailed_beat_plan Where bit_plan_id=0 and is_edit='edit' and date(date_of_visit)=date(now()) and frequency='".$frequency."' ) as temp_count,sales_rep_id from sales_rep_detailed_beat_plan Where date(date_of_visit)=date(now()) and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)= date(now()))and frequency='Alternate Tuesday' ) A UNION SELECT count(store_id) as unplan,sales_rep_id from sales_rep_detailed_beat_plan Where date(date_of_visit)=date(now()) and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)<>date(now())) and frequency='".$frequency."' UNION SELECT count(store_id) as unplan,sales_rep_id from sales_rep_detailed_beat_plan Where date(date_of_visit)=date(now()) and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)<>date(now()))and is_edit='edit' and frequency='".$frequency."')J where sales_rep_id<>'2'";
        $query=$this->db->query($sql);
        return $query->result();
}
...
function get_sales_rep1_email($frequency=''){
        ...
        $sql = "select count(distinct(sales_rep_id)) as total_sales_rep from sales_rep_beat_plan where frequency='".$frequency."' ";
        $query=$this->db->query($sql);
        return $query->result();
}
function get_sales_rep2_email($frequency=''){
        ...
        $sql = "select count(distinct(sales_rep_id)) as present_sales_rep from sales_rep_detailed_beat_plan Where date(date_of_visit)=date(now()) and frequency='".$frequency."'";
        $query=$this->db->query($sql);
        return $query->result();
}
