...
function get_distributor_out_sku_details($from_date, $to_date) {
    ...
    $sql = "select AA.*, WEEK(invoice_date,1)-WEEK(STR_TO_DATE(concat(YEAR(invoice_date),'-',MONTH(invoice_date),'-',1),'%Y-%m-%d'),1)+1 as dweek, 
            ...
            where AA.class!='sample' or AA.class is null order by invoice_date";
...
function generate_production_data_report() {
    ...
    $sql = "select C.*, D.depot_name from 
            ...
            (select A.*, B.short_name as product_name from 
