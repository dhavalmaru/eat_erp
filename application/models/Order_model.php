...
class Order_model Extends CI_Model{
    ...
    function generate_sodexo_upload_file($action='') {
        $from_date = '2001-01-01 00:00:00';

        $sql = "select * from sodexo_upload_files order by file_date desc limit 1";
        $result = $this->db->query($sql)->result();
        if(count($result)>0) {
            if(isset($result[0]->file_date)) {
                $from_date = $result[0]->file_date;
            }
        }
        ...
        $date1 = date('d-m-Y_His');
        $filename = 'Export_'.$date1.'.xlsx';

        $sql = "select * from sodexo_transaction where transaction_state='AUTHORIZED' and modified_on>'$from_date' and modified_on<='$to_date'";
        $result = $this->db->query($sql)->result();
        if(count($result)>0) {
            ...
            $tr = '<tr style="font-weight: bold;"><td>ID</td><td>Name</td><td>Command</td><td>Price: Total</td><td>Line: Type</td><td>Transaction: Kind</td><td>Transaction: Amount</td></tr>';

            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, "ID");
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, "Name");
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, "Command");
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, "Price: Total");
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, "Line: Type");
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, "Transaction: Kind");
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, "Transaction: Amount");
            ...
            for($i=0; $i<count($result); $i++){
                ...
                $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, '#'.$result[$i]->request_id);
                $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, 'UPDATE');
                $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, $result[$i]->amount);
                $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, 'Transaction');
                $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, 'sale');
                $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col++].$row, $result[$i]->authorised_amount);
            ...
            $objPHPExcel->getActiveSheet()->getStyle('A1:'.$col_name[6].$row)->applyFromArray(array(
                'borders' => array(
                    'allborders' => array(
            ...
            $path = $this->config->item('upload_path').'Sodexo_upload_files/';
            $upload_path = $this->config->item('upload_path').'Sodexo_upload_files';
            ...
            $this->db->insert('sodexo_upload_files', $data);
            ...
            if($action=="save") {
                ...
                $logarray['action']='Sodexo upload file generated.';
                $this->user_access_log_model->insertAccessLog($logarray);
    ...
    public function send_sodexo_upload_file() {
        ...
        $return_arr = $this->generate_sodexo_upload_file('save');
        ...
        if($reportpath!=''){
            ...
            $subject = 'Sodexo_upload_file_'.$report_date;
            ...
            if($mailSent==1){
                ...
                $logarray['module_name']='Reports';
                $logarray['cnt_name']='Reports';
                $logarray['action']='Sodexo upload file sent.';
                $this->user_access_log_model->insertAccessLog($logarray);
            }
        }
    }
