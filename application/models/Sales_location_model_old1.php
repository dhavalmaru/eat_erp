...
function get_data($status='', $id='',$frequency=''){
   ...
   $sql = "select G.*,H.date_of_visit,H.id as mid ,H.distributor_type,H.remarks,H.followup_date,H.distributor_type  from (select E.*,F.sales_rep_name from(select C.* from (select A.*,B.distributor_name ,B.distributor_name as store_name ,B.google_address,B.latitude,B.longitude,B.gst_number,B.margin,B.doc_document,B.document_name from 
...
public function save_session()
{
    ...
    if($orange_bar!=null)
    {
         ...
         if($type=='Box')
         {
            $temp_array['orange'] = $orange_bar.'_Box';
         }
         else
         {
            $temp_array['orange'] = $orange_bar.'_Bar';
         }
    }
    ...
    if($butterscotch_bar!=null)
    {
        ...
        if($type=='Box')
         {
            $temp_array['butterscotch'] = $butterscotch_bar.'_Box';
         }
         else
         {
            $temp_array['butterscotch'] = $butterscotch_bar.'_Bar';
         }
    }
    ...
    if($chocopeanut_bar!=null)
    {
        ...
        if($type=='Box')
         {
            $temp_array['chocopeanut'] = $chocopeanut_bar.'_Box';
         }
         else
         {
            $temp_array['chocopeanut'] = $chocopeanut_bar.'_Bar';
         }
    }
    ...
    if($bambaiyachaat_bar!=null)
    {
        $type = $this->input->post('type_4');
         if($type=='Box')
         {
            $temp_array['bambaiyachaat'] = $bambaiyachaat_bar.'_Box';
         }
         else
         {
            $temp_array['bambaiyachaat'] = $bambaiyachaat_bar.'_Bar';
         }
    }
    ...
    if($mangoginger_bar!=null)
    {
        ...
        if($type=='Box')
         {
            $temp_array['mangoginger'] = $mangoginger_bar.'_Box';
         }
         else
         {
            $temp_array['mangoginger'] = $mangoginger_bar.'_Bar';
         }
    }
    ...
    if($berry_blast_bar!=null)
    {
        ...
        if($type=='Box')
         {
            $temp_array['berry_blast_bar'] = $berry_blast_bar.'_Box';
         }
         else
         {
    ...
    if($chyawanprash_bar!=null)
    {
        ...
        if($type=='Box')
         {
            $temp_array['chyawanprash_bar'] = $chyawanprash_bar.'_Box';
         }
         else
         {
...
public function get_mtfollowup($id='',$temp_date='')
{
  ...
  $sql = "SELECT Distinct A.*, D.is_edit, D.is_visited  from 
            (Select A.dist_id as store_id, R.store_name, D.zone, A.zone_id, R.store_name as relation, C.location, 
                A.location_id, A.id as merchandiser_stock_id, 'Old' as distributor_type, distributor_status from 
            (SELECT * from merchandiser_stock Where date(followup_date)=$date  and m_id=$sales_rep_id)A
            left JOIN
...
public function get_gtfollowup($id='',$temp_date='')
{
   ...
   $sql = "SELECT Distinct A.*,B.*,D.is_edit,D.is_visited from 
            (SELECT A.* from 
            (SELECT id ,id as sales_rep_loc_id  ,zone_id,location_id,area_id,distributor_id as store_id,followup_date,distributor_name,distributor_type,distributor_status
...
function get_merchendiser_data($status='', $id='',$frequency=''){
        $cond2="";
      if($status!=""){
          $cond=" Where status='".$status."'";
      } else {
    ...
    $sql = "select Distinct G.*,H.date_of_visit,H.dist_id,H.id as mid,B.location,'Old' as distributor_type from (select E.*,F.sales_rep_name from(select C.*, D.google_address,D.latitude,D.longitude from (select A.*,B.store_name from 
      (".$table_name.$cond.' and sales_rep_id='.$sales_rep_id.") A 
      left join 
      (SELECT * FROM relationship_master where type_id ='4' or type_id='7') B 
      on (A.store_id=B.id))C
...
public function get_todaysorder()
{
  ...
  $sql = "select * from(SELECT C.distributor_name  from 
            (SELECT * from sales_rep_orders Where date(created_on)=date(now()) and sales_rep_id=$sales_rep_id and channel_type='GT')A
            Left join 
            (Select Distinct C.* FROM(
                                            Select B.* from (
                                                    Select concat('d_',A.id) as id , A.distributor_name ,A.google_address,A.latitude,A.longitude,'' as gst_number,'' as margin,'' as doc_document,' ' as document_name FROM
            ...
            On A.distributor_id=C.id
            Union
            SELECT C.distributor_name  from 
            ( SELECT * from sales_rep_orders Where date(created_on)=date(now()) and sales_rep_id=$sales_rep_id and channel_type='GT')A
            left join 
            (SELECT * from sales_rep_distributors) C
            On A.distributor_id=C.id
			Union
			SELECT C.store_name as distributor_name  from 
            ( SELECT * from sales_rep_orders Where date(created_on)=date(now()) and sales_rep_id=$sales_rep_id and channel_type='MT')A
            left join 
            (SELECT * from relationship_master) C
            On A.distributor_id=C.id)F WHERE distributor_name<>'Null'
            ";
    $query=$this->db->query($sql);
    return $query->result();
}
...
public function get_pendingsorder()
{
    ...
    $sql = "SELECT C.distributor_name  from 
                ( SELECT * from sales_rep_orders Where sales_rep_id=$sales_rep_id and date(created_on)>'2019-01-01' and channel_type='GT')A
                join 
                (Select Distinct C.* FROM(
                                                Select B.* from (
                                                        Select concat('d_',A.id) as id , A.distributor_name ,A.google_address,A.latitude,A.longitude,'' as gst_number,'' as margin,'' as doc_document,' ' as document_name FROM
