...
function get_sales_rep_email($frequency=''){
     ...
     $sql="
            select * from (SELECT A.*,(IFNULL(T.tem_visit_count, 0)+IFNULL(F.unplanned_count, 0)) as unplanned_count,E.p_call, B.sales_rep_name,  C.planned_count, D.actual_count,'Present' as emp_status   from 
                ...
                join (SELECT sales_rep_name,id from sales_rep_master where (status='Approved' || status='Active')) B 
...
function get_sales_rep1_email($frequency=''){
    ...
    $sql = "select count(distinct(b.sales_rep_id)) as total_sales_rep from sales_rep_beat_plan b left JOIN sales_rep_master s on s.id=b.sales_rep_id where (s.status='Approved' or s.status='Active') and b.frequency='".$frequency."' and b.sales_rep_id NOT IN(2) ";
    $query=$this->db->query($sql);
    return $query->result();
}
