            ...
            if($id!='' || $ref_id!='') { 
                ...
                if(strtoupper(trim($delivery_through))=='WHPL'){
                    ...
                    if($distributor_out_id==null || $distributor_out_id==''){
                        $sql = "insert into distributor_out (date_of_processing, invoice_no, depot_id, distributor_id, amount, tax, tax_per, tax_amount, final_amount, order_no, order_date, status, remarks, created_by, created_on, modified_by, modified_on, approved_by, approved_on, state, discount, delivery_status, cgst, sgst, igst, cgst_amount, sgst_amount, igst_amount, state_code, round_off_amount, invoice_amount, invoice_date, distributor_po_id,shipping_address ,distributor_consignee_id ,con_name ,con_address,con_city,con_pincode,con_state,con_country,con_state_code,con_gst_number) 
                            select date_of_po, '$invoice_no', depot_id, distributor_id, amount, tax, tax_per, tax_amount, final_amount, po_number, date_of_po, 'pending', '$remarks', '$curusr', '$now', '$curusr', '$now', '$curusr', '$now', state, discount, 'Pending', cgst, sgst, igst, cgst_amount, sgst_amount, igst_amount, state_code, round_off_amount, invoice_amount, '$invoice_date', '$id' ,shipping_address ,distributor_consignee_id ,con_name ,con_address,con_city,con_pincode,con_state,con_country,con_state_code,con_gst_number
...
function generate_po_delivery_report() {
    ...
    $data=$query->result();
     ...
     if(count($data)>0) {
        ...
        for($i=0; $i<count($data); $i++){
            ...
            if($po_id!=$prv_po_id){
                ...
                if(strtoupper(trim($data[$i]->delivery_status))=='DELIVERED' || strtoupper(trim($data[$i]->delivery_status))=='PENDING' || strtoupper(trim($data[$i]->delivery_status))=='CANCELLED'){
                    ...
                    if(strtoupper(trim($data[$i]->delivery_status))=='DELIVERED' || strtoupper(trim($data[$i]->delivery_status))=='PENDING'){
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+16].$row, '=SUBTOTAL(9,H'.$row.':P'.$row.')');
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+17].$row, $estimate_delivery_date);
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+18].$row, $data[$i]->tracking_id);
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+19].$row, $dispatch_date);
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+20].$row, $delivery_date);
                    } else if(strtoupper(trim($data[$i]->delivery_status))=='CANCELLED'){
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+15].$row, '=SUBTOTAL(9,H'.$row.':O'.$row.')');
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+16].$row, $data[$i]->delivery_remarks);
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+17].$row, $cancelled_date);
                    }
                }
            }
            ...
            if(strtoupper(trim($data[$i]->delivery_status))!='DELIVERED' && strtoupper(trim($data[$i]->delivery_status))!='PENDING' && strtoupper(trim($data[$i]->delivery_status))!='CANCELLED'){
                ...
                if($distributor_id!=$prv_distributor_id || $po_id!=$prv_po_id2){
                    ...
                    if(strtoupper(trim($data[$i]->delivery_through))=='WHPL'){
                        ...
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+15].$row, '=SUBTOTAL(9,H'.$row.':O'.$row.')');
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+16].$row, $estimate_delivery_date);
                    }
                }
        ...
        $objPHPExcel->setActiveSheetIndexByName('PO DELIVERD');
        $objPHPExcel->getActiveSheet()->getStyle('A1:'.$col_name[$col+20].$delivered_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
        ...
        $objPHPExcel->setActiveSheetIndexByName('PO CANCELLED');
        $objPHPExcel->getActiveSheet()->getStyle('A1:'.$col_name[$col+17].$cancelled_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('WHPL');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+20].$whpl_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Central Distributor');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+16].$central_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Sarvodaya Sales');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+16].$sarvodaya_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Allied');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+16].$allied_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Deepa');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+16].$deepa_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Heera Impex');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+16].$heera_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Steward');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+16].$steward_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Amoha');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+16].$amoha_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
...
public function po_summary_report()
{
    ...
    left join distributor_po_items E on (A.id=E.distributor_po_id) 
    left join box_product F on(E.type='Box' and E.item_id=F.box_id and E.item_id!='4' and E.item_id!='26' and E.item_id!='27') 
    where ((A.status='Approved' and A.delivery_status='Delivered' and date(date_of_po)=date(now()))
           or (A.status='Inactive' and A.delivery_status='Cancelled' )
           or (A.status='Approved' and A.delivery_status='')
           )) AA 
    group by AA.id, AA.date_of_po, AA.po_expiry_date, AA.po_number, AA.status, AA.remarks, 
        AA.estimate_delivery_date, AA.tracking_id, AA.dispatch_date, AA.delivery_status, AA.delivery_through, AA.delivery_date, 
        AA.delivery_remarks, AA.days_to_expiry, AA.distributor_id, AA.distributor_name, AA.store_name, 
        AA.location, AA.item_type, AA.item_id, AA.cancelled_date ) AA
    order by  CASE When AA.delivery_status='' Then 1 When AA.delivery_status='Delivered' Then 2 Else 3 end,AA.id, AA.distributor_id";
    ...
    if(count($data)>0) {
        ...
        for($i=0; $i<count($data); $i++){
            ...
            if($delivery_status!=$prv_delivery_status && $delivery_status=='Cancelled')
            {
             ...
             if($delivery_status!=$prv_delivery_status && $delivery_status=='Delivered')
            {
              echo "<tr></tr>
                    <tr>
                        <td cellpadding='23'>PO Delivered :</td>
                    </tr>";
            }
             ...
             if($po_id!=$prv_po_id){                
                    ...
                    echo "<tr>
                    <td>".$data[$i]->date_of_po."</td>
                    <td>".$data[$i]->po_expiry_date."</td>
                    <td>".$data[$i]->days_to_expiry."</td>
    ...
    $filename='Delivery_po_summary.xls';
    header('Content-Type: application/vnd.ms-excel');
    header('Content-Disposition: attachment;filename="'.$filename.'"');
    header('Cache-Control: max-age=0');
    $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
...
public function po_summary_report1()
{
    $sql = "Select * from 
    (select AA.id, AA.date_of_po, AA.po_expiry_date, AA.po_number, AA.status, AA.remarks, 
        AA.estimate_delivery_date, AA.tracking_id, AA.dispatch_date, AA.delivery_status, AA.delivery_through, 
    ...
    if(count($data)>0) {
        ...
        {
            $row =$row+2;
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col].$row, 'PO CANCELLED');
            $objPHPExcel->getActiveSheet()->getStyle($col_name[$col].$row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $this->excel->getActiveSheet()->getStyle($col_name[$col].$row)->getFont()->setUnderline(true);
