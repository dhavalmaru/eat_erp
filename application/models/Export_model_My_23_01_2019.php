...
function view_distributor_ledger_report($remark_visibility='') {
    ...
    if(count($data)>0) {
        ...
        $datatable=$datatable.'<tr>
                                    ...
                                    <td>'.format_money($diff_amount,2).'</td>
    ...
    if(count($data)>0) {
        ...
        for($i=0; $i<count($data); $i++){
            ...
            if($includes=='No' && (($data[$i]->r_closing_bal!=0 || (($openbal+$data[$i]->debit_amount)-$data[$i]->credit_amount)!=0)))
            {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('D'.$row, format_money($data[$i]->debit_amount,2));

                $objPHPExcel->getActiveSheet()->setCellValue('E'.$row, format_money($data[$i]->credit_amount,2));
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('H'.$row, format_money($data[$i]->r_closing_bal,2));
...
public function get_mt_stock_report($date='', $action='save', $r_zone_id='') {
    ...
    if(count($result)>0){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col].$row, '=SUM('.$col_name[$col+1].$row.':'.$col_name[$col+8].$row.')');
            $col = $col + 1;
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col].$row, '=IF('.$col_name[$col-32].$row.'<$F$'.$row.'/2,$F$'.$row.',0)');
            $col = $col + 1;
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col].$row, '=IF('.$col_name[$col-32].$row.'<$F$'.$row.'/2,$F$'.$row.',0)');
            $col = $col + 1;
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col].$row, '=IF('.$col_name[$col-32].$row.'<$F$'.$row.'/2,$F$'.$row.',0)');
