...
function get_sales_rep_order_email(){
    $sql = "select sum(A.amount)as od_value,sum(B.qty)as od_units from (select * from sales_rep_orders where date_of_processing='2018-10-11')A left join (select * from sales_rep_order_items) B on(A.id=B.sales_rep_order_id)";
    $query=$this->db->query($sql);
    return $query->result();
}

function get_sales_rep_total_visits_email(){
    $sql = "SELECT count(store_id) as unplan from sales_rep_detailed_beat_plan Where date(date_of_visit)=date('2018-10-11') and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)= date('2018-10-11')) UNION SELECT count(store_id) as unplan from sales_rep_detailed_beat_plan Where date(date_of_visit)=date('2018-10-11') and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)<>date('2018-10-11'))UNION SELECT count(store_id) as unplan from sales_rep_detailed_beat_plan Where date(date_of_visit)=date('2018-10-11') and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)<>date('2018-10-11'))and is_edit='edit' ;";
    $query=$this->db->query($sql);
    return $query->result();
}
...
function get_sales_rep_email($frequency=''){
	 ...
	 $sql="
			...
			(SELECT DISTINCT sales_rep_id,frequency from sales_rep_detailed_beat_plan Where frequency='Alternate Thursday' and date(date_of_visit)=date('2018-10-11')) A 
				LEFT join ( 
			Select `count`  as unplanned_count ,sales_rep_id,frequency from 
			(
				SELECT count(id) as `count` ,sales_rep_id,frequency from sales_rep_detailed_beat_plan Where frequency= 'Alternate Thursday' and store_id 
				IN (Select DISTINCT store_id from sales_rep_beat_plan Where frequency='Alternate Thursday' and date(created_on)=date('2018-10-11') ) 
				and date(date_of_visit)=date('2018-10-11') 
			...
			left join (SELECT count(id) as p_call,sales_rep_id from sales_rep_orders where date(date_of_processing)=date('2018-10-11')) E 
			...
			Where frequency= 'Alternate Thursday' and store_id IN (Select DISTINCT store_id from sales_rep_beat_plan Where frequency='Alternate Thursday'
			and date(created_on)<>date('2018-10-11') ) and date(date_of_visit)=date('2018-10-11') and is_edit='edit') B GROUP BY sales_rep_id 
			...
			SELECT count(*) as actual_count ,sales_rep_id from ( 
							...
							Where frequency= 'Alternate Thursday' and store_id IN (Select DISTINCT store_id from sales_rep_beat_plan Where frequency= 'Alternate Thursday'
							and date(created_on)<>date('2018-10-11') ) and date(date_of_visit)=date('2018-10-11') 
...
function get_sales_rep2_email(){
    $sql = "select count(distinct(sales_rep_id)) as present_sales_rep from sales_rep_detailed_beat_plan Where date(date_of_visit)=date('2018-10-11')";
    $query=$this->db->query($sql);
    return $query->result();
}
