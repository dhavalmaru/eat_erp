...
function generate_po_delivery_report() {
    ...
    (select AA.id, AA.date_of_po, AA.po_expiry_date, AA.order_no as po_number, AA.status, AA.remarks, 
        ...
        '' as store_name , AA.location, AA.item_type, AA.item_id, AA.cancelled_date, 
        ...
        case when (E.type='Box' and (E.item_id='4' or E.item_id='26' or E.item_id='27' or E.item_id='13' or E.item_id='14' or E.item_id='32')) then E.type else 'Bar' end as item_type, 
        case when E.type='Bar' then E.item_id when (E.type='Box' and (E.item_id='4' or E.item_id='26' or E.item_id='27' or E.item_id='13' or E.item_id='14' or E.item_id='32')) then E.item_id else F.product_id end as item_id, 
        case when E.type='Bar' then E.qty when (E.type='Box' and (E.item_id='4' or E.item_id='26' or E.item_id='27' or E.item_id='13' or E.item_id='14' or E.item_id='32')) then E.qty else (E.qty*F.qty) end as item_qty ,P.location
        ...
        Where A.status='Approved' and (A.delivery_status='Delivered Not Complete')
        ...
        where CASE When A.status = 'Inactive' Then distributor_po_id IS NOT NULL ELSE 1=1 end and (CASE When A.delivery_status='Delivered Not Complete' Then distributor_po_id IS NOT NULL ELSE 1=1 end)) AA 
        ...
        Select * from 
            ...
            case when (E.type='Box' and (E.item_id='4' or E.item_id='26' or E.item_id='27' or E.item_id='13' or E.item_id='14' or E.item_id='32')) then E.type else 'Bar' end as item_type, 
            case when E.type='Bar' then E.item_id when (E.type='Box' and (E.item_id='4' or E.item_id='26' or E.item_id='27' or E.item_id='13' or E.item_id='14' or E.item_id='32')) then E.item_id else F.product_id end as item_id, 
            case when E.type='Bar' then E.qty when (E.type='Box' and (E.item_id='4' or E.item_id='26' or E.item_id='27' or E.item_id='13' or E.item_id='14' or E.item_id='32')) then E.qty else (E.qty*F.qty) end as item_qty 
            ...
            order by AA.id, AA.distributor_id ) AA
                ...
                When (AA.delivery_status='GP Issued' and (AA.distributor_po_id IS NULL OR AA.distributor_po_id='')) Then 1
                When (AA.delivery_status='Delivered' OR AA.delivery_status='Pending' OR AA.delivery_status='Delivered Not Complete' and AA.status='Approved') Then 2 Else 3 end,AA.id, AA.distributor_id";
    $query=$this->db->query($sql);
    $data=$query->result();
     ...
     if(count($data)>0) {
            ...
            if($po_id!=$prv_po_id){
                ...
                if(strtoupper(trim($data[$i]->delivery_status))=='DELIVERED' || strtoupper(trim($data[$i]->delivery_status))=='PENDING' || strtoupper(trim($data[$i]->delivery_status))=='CANCELLED'){
                    ...
                    if(strtoupper(trim($data[$i]->delivery_status))=='DELIVERED' || strtoupper(trim($data[$i]->delivery_status))=='PENDING'){
                        ...
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+25].$row, $comments);
                    }
                }
            }

            if(($data[$i]->delivery_status='Approved' OR $data[$i]->delivery_status='Inactive') && strtoupper(trim($data[$i]->delivery_status))=='DELIVERED' || strtoupper(trim($data[$i]->delivery_status))=='PENDING' || strtoupper(trim($data[$i]->delivery_status))=='CANCELLED'){
            ...
            if(strtoupper(trim($data[$i]->delivery_status))!='DELIVERED' && strtoupper(trim($data[$i]->delivery_status))!='PENDING' && strtoupper(trim($data[$i]->delivery_status))!='CANCELLED'){
                ...
                if($distributor_id!=$prv_distributor_id || $po_id!=$prv_po_id2){
                    ...
                    if(strtoupper(trim($data[$i]->delivery_through))=='WHPL'){
                        ...
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+22].$row, '=SUBTOTAL(9,H'.$row.':V'.$row.')');
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+23].$row, $estimate_delivery_date);
                        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+24].$row, $comments);
                    }
                }
        ...
        $objPHPExcel->setActiveSheetIndexByName('PO DELIVERD');
        $objPHPExcel->getActiveSheet()->getStyle('A1:'.$col_name[$col+27].$delivered_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
        ...
        $objPHPExcel->setActiveSheetIndexByName('PO CANCELLED');
        $objPHPExcel->getActiveSheet()->getStyle('A1:'.$col_name[$col+25].$cancelled_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('WHPL');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+27].$whpl_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Central Distributor');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+24].$central_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Sarvodaya Sales');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+24].$sarvodaya_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Allied');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+24].$allied_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Heera Impex');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+24].$heera_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Steward');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+24].$steward_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->setActiveSheetIndexByName('Amoha');
        $objPHPExcel->getActiveSheet()->getStyle('A9:'.$col_name[$col+24].$amoha_row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
...
function send_po_delivery_report() {
    $sql = "select AA.id, AA.date_of_po, AA.po_expiry_date, AA.po_number, AA.status, AA.remarks, 
                ...
                AA.delivery_date, AA.delivery_remarks, AA.days_to_expiry, AA.distributor_id, AA.distributor_name, 
                AA.store_name, AA.location, AA.item_type, AA.item_id, AA.cancelled_date, 
                sum(AA.item_qty) as tot_qty,AA.comments from 
            (select A.*, datediff(A.po_expiry_date, curdate()) as days_to_expiry, B.distributor_name, C.store_name, D.location, 
            case when (E.type='Box' and (E.item_id='4' or E.item_id='26' or E.item_id='27' or E.item_id='13' or E.item_id='14' or E.item_id='32')) then E.type else 'Bar' end as item_type, 
            case when E.type='Bar' then E.item_id when (E.type='Box' and (E.item_id='4' or E.item_id='26' or E.item_id='27' or E.item_id='13' or E.item_id='14' or E.item_id='32')) then E.item_id else F.product_id end as item_id, 
            case when E.type='Bar' then E.qty when (E.type='Box' and (E.item_id='4' or E.item_id='26' or E.item_id='27' or E.item_id='13' or E.item_id='14' or E.item_id='32')) then E.qty else (E.qty*F.qty) end as item_qty 
...
public function po_summary_report()
{
    ...
    (select AA.id, AA.date_of_po, AA.po_expiry_date, AA.order_no as po_number, AA.status, AA.remarks, 
        ...
        left join distributor_out_items E on (A.id=E.distributor_out_id) 
        left join box_product F on(E.type='Box' and E.item_id=F.box_id and E.item_id NOT IN (4,26,27,13,14,32)) 
        where CASE When A.status = 'Inactive' Then distributor_po_id IS NOT NULL and date(A.modified_on)=date(now())  ELSE 1=1 end
    ...
    if(count($data)>0) {
        ...
        <th style='width: 60px;'>cookies CR</th>
        <th style='width: 60px;'>cookies CH</th>
        <th style='width:81px;'>cookies DR CH</th>
        <th style='width:30px;'>Total</th>
        <th style='width: 103px;'>Appointment Date</th>
