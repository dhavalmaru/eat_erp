...
function get_data2($status='', $id=''){
	...
	$sql = "SELECT  Distinct concat('d_',A.id) as id, A.sales_rep_id, A.distributor_name, 
            ...
            where A.status='approved' and A.type_id=3  and A.class='normal' and A.distributor_name<>'' group by distributor_name
        Union All
        select concat('s_',id) as id, B.sales_rep_id, B.distributor_name, 
            ...
            B.modified_by, B.modified_on 
                from sales_rep_distributors B where B.distributor_name<>''
         group by distributor_name order by distributor_name asc
              ";
    $query=$this->db->query($sql);
    return $query->result();
}
