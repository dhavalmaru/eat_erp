...
function get_data($status='', $id='',$frequency=''){
    ...
    //utf8mb4_unicode_ci
    $sql = "select G.*,H.date_of_visit,H.id as mid , H.remarks,H.followup_date,H.distributor_type,H.location_id,H.area_id,H.zone_id  from (select E.*,F.sales_rep_name from(select C.* from (select A.*,B.distributor_name ,B.distributor_name as store_name ,B.google_address,B.latitude,B.longitude,B.gst_number,B.margin,B.doc_document,B.document_name from 
        (".$table_name.") A 
        left join 
...
function save_data($id='',$status=''){
    ...
    if($this->input->post('distributor_type')=='New'){
        ...
        echo  "Entered IN else Condition";
        if($mid!="")
        {
            $this->db->where('id', $mid);
            $this->db->update('sales_rep_location',$data);
        ...
        else
        {
            ...
            if($ispermenant=='Yes' || $place_order=='Yes')
            {
                ...
                echo '<br>'.$sql = "UPDATE sales_rep_beat_plan m1
