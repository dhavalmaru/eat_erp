...
function get_distributor_out_details($from_date, $to_date) {
    $sql = "select I.*, K.location from  
            ...
            (select *,MONTHNAME(STR_TO_DATE(month(date_of_processing),'%m')) as dmonth from distributor_out where status='Approved' and date_of_processing>='$from_date' and date_of_processing<='$to_date') C 
            left join 
...
function generate_sale_invoice_report() {
    ...
    if(count($data)>0) {
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('P'.$row, $data[$i]->remarks);
            $row=$row+1;
        }
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1'.':P'.$row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
...
function get_distributor_out_sku_details($from_date, $to_date) {
    $sql = "select L.*, J.item_name, J.quantity from
            ...
            (select *,MONTHNAME(STR_TO_DATE(month(date_of_processing),'%m')) as dmonth from distributor_out where status='Approved' and date_of_processing>='$from_date' and date_of_processing<='$to_date') A 
            left join 
            ...
            left join 
            (select id, 'Raw Material' as type, rm_name as item_name, null as quantity from raw_material_master where status='Approved' 
            union all 
            select id, 'Bar' as type, product_name as item_name, null as quantity from product_master where status='Approved' 
            union all 
            select m.id, 'Box' as type, m.box_name as item_name, sum(p.qty) as quantity from box_master m left join box_product p on m.id=p.box_id where m.status='Approved' group by m.id) J 
...
function generate_sale_invoice_sku_report() {
    $from_date = formatdate($this->input->post('from_date'));
    $to_date = formatdate($this->input->post('to_date'));
    ...
    if(count($data)>0) {
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('J'.$row, $tax);
            $cstamt=$tax+$data[$i]->item_amount;
            $objPHPExcel->getActiveSheet()->setCellValue('K'.$row, $cstamt);
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('N'.$row, $data[$i]->distributor_type);
            $objPHPExcel->getActiveSheet()->setCellValue('O'.$row, $data[$i]->location);
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('T'.$row, $barquantity);
            $objPHPExcel->getActiveSheet()->setCellValue('U'.$row, $data[$i]->remarks);
            $objPHPExcel->getActiveSheet()->setCellValue('V'.$row, $data[$i]->dmonth);
            $row=$row+1;
            ...
            if($i<count($data)-1){
                if($data[$i]->invoice_no!=$data[$i+1]->invoice_no){
                    $objPHPExcel->getActiveSheet()->setCellValue('A'.$row, $data[$i]->date_of_processing);
                    $objPHPExcel->getActiveSheet()->setCellValue('B'.$row, $data[$i]->invoice_no);
                    $objPHPExcel->getActiveSheet()->setCellValue('C'.$row, 'Round Off Amount');
                    $round_off_amt=round($data[$i]->final_amount)-$data[$i]->final_amount;
                    $objPHPExcel->getActiveSheet()->setCellValue('K'.$row, $round_off_amt);
                    $objPHPExcel->getActiveSheet()->setCellValue('L'.$row, $data[$i]->depot_name);
                    $objPHPExcel->getActiveSheet()->setCellValue('M'.$row, $data[$i]->distributor_name);
                    $objPHPExcel->getActiveSheet()->setCellValue('N'.$row, $data[$i]->distributor_type);
                    $objPHPExcel->getActiveSheet()->setCellValue('O'.$row, $data[$i]->location);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1'.':V'.$row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
