...
function get_distributor_out_sku_details($from_date, $to_date) {
    ...
    $sql = "select AA.*, WEEK(invoice_date,1)-WEEK(STR_TO_DATE(concat(YEAR(invoice_date),'-',MONTH(invoice_date),'-',1),'%Y-%m-%d'),1)+1 as dweek, 
            ...
            from distributor_out A 
                ...
                left join location_master G on(D.location_id=G.id) 
                left join area_master H on(D.area_id=H.id) 
            where A.invoice_date>='$from_date' and A.invoice_date<='$to_date' and A.status='Approved') AA 
            left join 
            (select id, 'Bar' as type, product_name as item_name, null as quantity, short_name from product_master 
                where status='Approved' 
            union all 
...
function generate_distributorwise_report() {
    ...
    if(count($data)>0) {
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1:'.$col_name[$col+25].$row)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                )
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1:Z1')->getFill()->applyFromArray(array(
            'type' => PHPExcel_Style_Fill::FILL_SOLID,
            'startcolor' => array(
                'rgb' => 'D9D9D9'
            )
