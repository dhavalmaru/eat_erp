...
class Distributor_sale extends CI_Controller{
    ...
    public function download_csv()
    {
           ...
           /*for ($col = 0; $col <= PHPExcel_Cell::columnIndexFromString($objPHPExcel->getActiveSheet()->getHighestDataColumn()); $col++) {
            ...
            $filename='superstockis_upload.xls';
    ...
    public function upload_file()
    {
           ...
           if($error!='')
            ...
            else
            {
                $sql = "select G.*,H.location from (select E.*, F.store_name 
                    from(
                    select C.*,D.id as to_distributor_id, D.store_id,D.location_id from (
                    select A.distributor_name,A.id as dist_id ,B.* from(select *  from distributor_master where class='Super Stockist' )A
                    left join(select * from zone_master where type_id='4' or type_id='7' ) B 
                    on( A.zone_id=B.id))C
                    left join
                    ...
                    Where
                    G.distributor_name = '$distributor' AND
                    G.zone = '$zone' AND 
                ...
                if(count($result)>0)
                {
                        ...
                        $distributor_id = $result[0]->dist_id;
                        $location_id = $result[0]->location_id;
                        $zone_id = $result[0]->id;
                        $store_id = $result[0]->store_id;
                        $to_distributor_id = $result[0]->to_distributor_id;


                        $result2 = "Select * from  (select id, 'Bar' as type, product_name as item_name, rate,    product_name,status from product_master where status='Approved'  union all select m.id, 'Box' as type, m.box_name as item_name, m.rate,   m.box_name,status from box_master m )A  Where  item_name= '$item'  group by A.id";
        ...
        if($objerror==1)
        {
            $filename='superstockis_upload_errors'.'_'.time().'.xls';
            header('Content-Type: application/vnd.ms-excel');
            header('Content-Disposition: attachment;filename="'.$filename.'"');
            header('Cache-Control: max-age=0');
            $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
