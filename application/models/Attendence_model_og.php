...
{	
	...
	$sql = "Select A.*,B.adjusted_late_marks,B.adjusted_late_marks_count,B.employee_adjusted_time,B.adjusted_effective_time from (Select A.emp_id,A.emp_no,A.emp_name,A.date,A.`first_in`,
		...
		When emp_status='WeeklyOff' THEN 'WeeklyOff' 
		When emp_status='Absent' THEN 'Absent' 
		When emp_status='Present' Then (Case When CAST(last_out As Time)<CAST(first_in As Time ) Then (CAST(ADDTIME(TIMEDIFF(last_out,'00:00'),TIMEDIFF('24:00:00',first_in)) as Time))  Else cast(TIMEDIFF(last_out, first_in) as time)  end )  end as effective_time,
		...
		left join
			...
			When emp_status='WeeklyOff' THEN 'WeeklyOff' 
			When emp_status='Absent' THEN 'Absent' 
			When emp_status='Present' Then (Case When (CAST(adjusted_out_time As Time)
			>=CAST('00:00:00' As Time ) AND CAST(adjusted_out_time As Time)
			<=CAST('08:00:00' As Time )) Then (CAST(ADDTIME(TIMEDIFF(adjusted_out_time,'00:00'),TIMEDIFF('24:00:00',adjusted_in_time)) as Time))  Else cast(TIMEDIFF(adjusted_out_time, adjusted_in_time) as time)  end )  end as effective_time,
