...
function upload_distributor_balance_ledger_report() {
    ...
    $sql = "Select A.*,B.r_tally_name,B.r_closing_bal from 
                ...
                Where A.tally_report_id=(SELECT max(tally_report_id) from tally_report 
                Where distributer_id=A.distributer_id and 
                added_on=(SELECT max(added_on) from tally_report Where distributer_id=A.distributer_id))) B
...
public function send_exception_report() {
    ...
    $sql = "select 'Sales' as temp_col, sum(case when date(A.created_on)=date('".$date."') then 1 else 0 end) as entry_done, 
            ...
            sum(case when date(A.date_of_processing)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (delivery_status='Pending' or delivery_status='GP Issued' or delivery_status='Delivered Not Complete' or delivery_status='Delivered') and (A.ref_id is not null and A.ref_id <> '') then 1 else 0 end) as prior_pending 
            from distributor_out A 
            ...
            sum(case when date(A.date_of_deposit)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (A.ref_id is not null and A.ref_id <> '') then 1 else 0 end) as prior_pending 
            ...
            sum(case when date(A.date_of_processing)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (A.ref_id is null or A.ref_id = '') then 1 else 0 end) as pending, 
            sum(case when date(A.date_of_processing)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (A.ref_id is not null and A.ref_id <> '') then 1 else 0 end) as prior_pending 
            ...
            sum(case when date(A.date_of_transaction)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (A.ref_id is null or A.ref_id = '') then 1 else 0 end) as pending, 
            sum(case when date(A.date_of_transaction)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (A.ref_id is not null and A.ref_id <> '') then 1 else 0 end) as prior_pending 
    ...
    $sql = "select 'Sales' as temp_col, A.id, A.date_of_processing as ref_date, A.invoice_no as ref_no, A.distributor_id, A.invoice_amount as amount, A.modified_by, A.modified_on, B.distributor_name, concat(ifnull(C.first_name,''),' ',ifnull(C.last_name,'')) as modifiedby 
        ...
        where A.date_of_processing is not null and A.distributor_id!='1' and A.distributor_id!='189' and date(A.date_of_processing)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (A.delivery_status='Pending' or A.delivery_status='GP Issued' or A.delivery_status='Delivered Not Complete' or A.delivery_status='Delivered') and (A.ref_id is not null and A.ref_id <> '') 

        union all 

        select 'Payment Received' as temp_col, A.id, A.date_of_deposit as ref_date, B.ref_no, B.distributor_id, B.payment_amount as amount, A.modified_by, A.modified_on, C.distributor_name, concat(ifnull(D.first_name,''),' ',ifnull(D.last_name,'')) as modifiedby 
        ...
        where A.date_of_deposit is not null and date(A.date_of_deposit)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (A.ref_id is not null and A.ref_id <> '') 

        union all 

        select 'Sales Return' as temp_col, A.id, A.date_of_processing as ref_date, A.sales_return_no as ref_no, A.distributor_id, A.final_amount as amount, A.modified_by, A.modified_on, B.distributor_name, concat(ifnull(C.first_name,''),' ',ifnull(C.last_name,'')) as modifiedby 
        ...
        where A.date_of_processing is not null and date(A.date_of_processing)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (A.ref_id is not null and A.ref_id <> '') 

        union all 

        select 'Sample' as temp_col, A.id, A.date_of_processing as ref_date, A.voucher_no as ref_no, A.distributor_id, A.final_amount as amount, A.modified_by, A.modified_on, B.distributor_name, concat(ifnull(C.first_name,''),' ',ifnull(C.last_name,'')) as modifiedby 
        from distributor_out A left join distributor_master B on (A.distributor_id=B.id) left join user_master C on (A.modified_by=C.id) 
        where A.date_of_processing is not null and (A.distributor_id='1' or A.distributor_id='189') and date(A.date_of_processing)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (A.delivery_status='Pending' or A.delivery_status='GP Issued' or A.delivery_status='Delivered Not Complete' or A.delivery_status='Delivered') and (A.ref_id is not null and A.ref_id <> '') 

        union all 

        select 'Credit Notes' as temp_col, A.id, A.date_of_transaction as ref_date, A.ref_no, A.distributor_id, A.amount, A.modified_by, A.modified_on, B.distributor_name, concat(ifnull(C.first_name,''),' ',ifnull(C.last_name,'')) as modifiedby 
        ...
        where A.date_of_transaction is not null and date(A.date_of_transaction)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (A.ref_id is not null and A.ref_id <> '') and transaction = 'Credit Note' 

        union all 

        select 'Debit Notes' as temp_col, A.id, A.date_of_transaction as ref_date, A.ref_no, A.distributor_id, A.amount, A.modified_by, A.modified_on, B.distributor_name, concat(ifnull(C.first_name,''),' ',ifnull(C.last_name,'')) as modifiedby 
        from credit_debit_note A left join distributor_master B on (A.distributor_id=B.id) left join user_master C on (A.modified_by=C.id) 
        where A.date_of_transaction is not null and date(A.date_of_transaction)<date('".$date."') and (A.status = 'Pending' or A.status = 'Deleted') and (A.ref_id is not null and A.ref_id <> '') and transaction = 'Debit Note'";
    $query = $this->db->query($sql);
    $result = $query->result();
    $filename = '';
    $path = '';
    if(count($result)>0) {
        ...
        // $path  = 'C:/wamp64/www/eat_erp_test/assets/uploads/exception_reports/';
...
function get_sales_rep_order_email($date=''){
    $sql = "select sum(A.amount)as od_value,sum(B.qty)as od_units from (select * from sales_rep_orders where date_of_processing='$date')A left join (select * from sales_rep_order_items) B on(A.id=B.sales_rep_order_id)";
    $query=$this->db->query($sql);
    return $query->result();
}

function get_sales_rep_total_visits_email($date=''){
    $sql = "SELECT count(store_id) as unplan from sales_rep_detailed_beat_plan Where date(date_of_visit)=date('$date') and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)= date('$date')) UNION SELECT count(store_id) as unplan from sales_rep_detailed_beat_plan Where date(date_of_visit)=date('$date') and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)<>date('$date'))UNION SELECT count(store_id) as unplan from sales_rep_detailed_beat_plan Where date(date_of_visit)=date('$date') and store_id IN ( Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)<>date('$date'))and is_edit='edit' ;";
    $query=$this->db->query($sql);
    return $query->result();
}

function get_sales_rep_email($frequency='',$date=''){
	  $day = date('l');
        $m = date('F');
        $year = date('Y');
	  ...
	  $sql="SELECT A.*,F.unplanned_count ,E.p_call, B.sales_rep_name,  C.planned_count, D.actual_count  from 
			(SELECT DISTINCT sales_rep_id,frequency from sales_rep_detailed_beat_plan Where date(date_of_visit)=date('$date')) A 
				LEFT join ( 
			Select `count`  as unplanned_count ,sales_rep_id,frequency from 
			(
				SELECT count(id) as `count` ,sales_rep_id,frequency from sales_rep_detailed_beat_plan Where store_id 
				IN (Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)=date('$date') ) 
				and date(date_of_visit)=date('$date') 
			...
			left join (SELECT count(id) as p_call,sales_rep_id from sales_rep_orders where date(date_of_processing)=date('$date')) E 
			on A.sales_rep_id=E.sales_rep_id
			LEFT JOIN 
			(
			SELECT count(*) as planned_count ,sales_rep_id from ( SELECT store_id,sales_rep_id,date_of_visit from sales_rep_detailed_beat_plan Where  store_id IN (Select DISTINCT store_id from sales_rep_beat_plan Where 
			date(created_on)<>date('$date') ) and date(date_of_visit)=date('$date') and is_edit='edit') B GROUP BY sales_rep_id 
			...
			SELECT count(*) as actual_count ,sales_rep_id from ( 
							...
							Where  store_id IN (Select DISTINCT store_id from sales_rep_beat_plan Where date(created_on)<>date('$date') ) and date(date_of_visit)=date('$date') 
...
function get_sales_rep2_email($date=''){
    $sql = "select count(distinct(sales_rep_id)) as present_sales_rep from sales_rep_detailed_beat_plan Where date(date_of_visit)=date('$date')";
    $query=$this->db->query($sql);
    return $query->result();
}
...
function generate_sales_representative_daily_report() {
		...
		$frequency='';
        $data = $this->get_sales_rep_email($frequency,$date);
				
        $data1 = $this->get_sales_rep1_email();
        $data2 = $this->get_sales_rep2_email($date);
        $orders= $this->get_sales_rep_order_email($date);
        $total_visits = $this->get_sales_rep_total_visits_email($date);
    ...
    if(count($data)>0) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col].$row1, "");

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+1].$row1,"Attendance");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+2].$row1,"");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+3].$row1,"Total");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+4].$row1,"Total");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+5].$row1,"Total");
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+6].$row1,"Total");
        ...
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col].$row1, "");
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+1].$row1,$data2[0]->present_sales_rep.'/'.$data1[0]->total_sales_rep);
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+2].$row1,"");
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+3].$row1,$total_visits[1]->unplan);
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+4].$row1,$total_visits[2]->unplan);
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+5].$row1,$total_visits[0]->unplan);
		
		$total_calls=$total_visits[0]->unplan + $total_visits[2]->unplan;
		
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+6].$row1,$total_calls);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+1].$row, "Name Of Employee");

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+2].$row, "Beat Name");

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+3].$row, "Planned Vists");

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+4].$row, "Acutal Vists");

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+5].$row, "Unplanned Visits");

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+6].$row, "Total Calls");

		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+7].$row, "Deviation from Route Plan");

		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+8].$row, "Productive Calls");

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+9].$row, "% Productivity");

		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+10].$row, "Total Order Units");

		
		

		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+11].$row, "Total Order Value");

		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+12].$row, "Attendence Time");

		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+13].$row, "First Call");

	    $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+14].$row, "Last Call");

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+15].$row, "Time Spent on Field"); 

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+16].$row, "End of Day Reporting");  

        $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+17].$row, "Stock Zero No order Placed");  

		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+18].$row, "Follow Up");  
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+19].$row, "Outlet Closed");  
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+20].$row, "Owner No Available");  
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+21].$row, "Excess Stock");  
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+22].$row, "Margin Issue");  
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+23].$row, "Service Issues");  
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+24].$row, "Delivery Issues"); 
		$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+25].$row, "No Follow up sales"); 
        ...
        for($i=0; $i<count($data); $i++){
            ...
            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+2].$row, $data[$i]->frequency);

            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+3].$row, $data[$i]->actual_count);

            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+4].$row, $data[$i]->planned_count);

            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+5].$row, $data[$i]->unplanned_count);

            $objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+6].$row, $single_total_calls);

			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+7].$row, round($deviation,2));

			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+8].$row, (isset($data[$i]->p_call)?$data[$i]->p_call:0));
			...
			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+11].$row, " ");

			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+12].$row, " ");

			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+13].$row, " ");

			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+14].$row, " ");

			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+15].$row, " ");

			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+16].$row, " ");
			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+17].$row, " ");
			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+18].$row, " ");
			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+19].$row, " ");
			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+20].$row, " ");
			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+21].$row, " ");
			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+22].$row, " ");
			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+23].$row, " ");
			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+24].$row, " ");
			$objPHPExcel->getActiveSheet()->setCellValue($col_name[$col+25].$row, " ");
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A3:'.$col_name[$col+25].$row)->applyFromArray(array(
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A3:Z3')->getFill()->applyFromArray(array(
        ...
        $filename='Sales_rep_daily_tracker.xls';
        ...
        $logarray['action']='Sales Representative Daily report generated.';
