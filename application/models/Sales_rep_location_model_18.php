...
function get_locations($zone_id='', $area_id='',$id=''){
    $cond = '';
    if($zone_id!=''){
        $cond = $cond . " and zone_id = '$zone_id'";
    }
...
public function get_store_name($id='',$zone_id='')
{
    ...
    echo 'get_store_name'.$sql = "select * from (SELECT store_name,s.id as store_id,location_id,zone_id from   store_master s JOIN relationship_master rm on s.store_id=rm.id) A ".$cond;
