...
public function get_todaysorder()
{
  ...
  $sql = "SELECT C.distributor_name  from 
            ( SELECT * from sales_rep_orders Where date(created_on)=date(now()) and sales_rep_id=$sales_rep_id )A
            Left join 
            (Select Distinct C.* FROM(
                                            Select B.* from (
                                                    Select concat('d_',A.id) as id , A.distributor_name ,A.google_address,A.latitude,A.longitude,'' as gst_number,'' as margin,'' as doc_document,' ' as document_name FROM
...
public function get_pendingsorder()
{
    ...
    $sql = "SELECT C.distributor_name  from 
                ( SELECT * from sales_rep_orders Where sales_rep_id=$sales_rep_id and date(created_on)>'2019-01-01')A
                join 
                (Select Distinct C.* FROM(
                                                Select B.* from (
                                                        Select concat('d_',A.id) as id , A.distributor_name ,A.google_address,A.latitude,A.longitude,'' as gst_number,'' as margin,'' as doc_document,' ' as document_name FROM
