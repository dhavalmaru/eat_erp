...
function get_purchase_order_nos($vendor_id){
    $sql = "select * from purchase_order where status='Approved' and vendor_id = '$vendor_id' and po_status NOT IN('Raw Material IN','Closed')";
    $query=$this->db->query($sql);
    return $query->result();
}

function get_purchase_order_nos_by_status($vendor_id){
    $sql = "select * from purchase_order where status='Approved' and vendor_id = '$vendor_id' and po_status NOT IN('Advance','Closed')";
    $query=$this->db->query($sql);
    return $query->result();
}
    ...
    if(count($result)>0){
        ...
        if(count($result)>0){
            ...
            if($email_id!=''){
                ...
                //Link: <a><input type="submit" value="View Purchase Order" style="border: none; background: none; color: blue; text-decoration: underline; cursor: pointer;"></a>
                ...
                $message = '<html><head></head><body>Dear, '.$vendor_name.',<br /><br />' .
                            ...
                            '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Total: Rs '.$total_amount.'<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;--------------------------------------------------------------------------' .
                            '</body></html>';
                $path  = '/home/eatangcp/public_html/test/assets/uploads/'; 
...
function generate_purchase_order($id,$pdfFilePath='') {
    $now=date('Y-m-d H:i:s');
    $curusr=$this->session->userdata('session_id');
