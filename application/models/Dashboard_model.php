...
class Dashboard_model extends CI_Model {
    ...
    //     $sql="select DD.depot_id, DD.product_id, sum(DD.tot_qty) as tot_qty, DD.depot_name, DD.short_name, DD.denominations, sum(DD.total_bars) as total_bars from 

    //         (select H.*, I.short_name, 1 as denominations, tot_qty as total_bars from 
    //         (select F.*, G.depot_name from 
    //         (select E.depot_id, E.product_id, sum(tot_qty) as tot_qty from 
    //         (select C.depot_id, C.product_id, ifnull(C.qty_in,0)-ifnull(D.qty_out,0) as tot_qty from 
    //         (select AA.depot_id, AA.product_id, sum(AA.qty) as qty_in from 
    ...
    //         select CC.depot_id, CC.product_id, sum(CC.tot_qty) as tot_qty, CC.depot_name, CC.short_name, CC.denominations, sum(CC.total_bars) as total_bars from 
    //         (select AA.depot_id, BB.product_id, AA.tot_qty, AA.depot_name, BB.short_name, BB.qty as denominations, ifnull(AA.tot_qty,0)*ifnull(BB.qty,0) as total_bars from 
    //         (select H.*, I.short_name from
    //         (select F.*, G.depot_name from 
    //         (select E.depot_id, E.box_id, sum(tot_qty) as tot_qty from 
    //         (select C.depot_id, C.box_id, ifnull(C.qty_in,0)-ifnull(D.qty_out,0) as tot_qty from 
    //         (select AA.depot_id, AA.box_id, sum(AA.qty) as qty_in from 
    ...
    //         on (H.box_id=I.id)) AA 
    //         left join 
    //         (select A.*, B.short_name from box_product A left join product_master B on (A.product_id = B.id)) BB 
    //         on (AA.box_id = BB.box_id)) CC 
    //         where CC.tot_qty > 0
    //         group by CC.depot_id, CC.product_id, CC.depot_name, CC.short_name, CC.denominations) DD 

    //         where DD.tot_qty > 0
    //         group by DD.depot_id, DD.product_id, DD.depot_name, DD.short_name, DD.denominations 
    ...
    function get_total_product_stock() {
        $sql="select DD.depot_id, DD.product_id, sum(DD.tot_qty) as tot_qty, DD.depot_name, DD.short_name, sum(DD.total_bars) as total_bars from 
            ...
            where DD.tot_qty > 0
            group by DD.depot_id, DD.product_id, DD.depot_name, DD.short_name
            order by  DD.depot_id,DD.product_id";
        $query=$this->db->query($sql);
        return $query->result();
    }
    ...
    //     $sql="select J.*, K.short_name from 
    ...
    //         on (J.product_id=K.id) order by J.distributor_name";
    //     $query=$this->db->query($sql);
    //     return $query->result();
    // }

    function get_product_stock_for_distributor() {
        $sql="select J.distributor_id, J.distributor_name, J.product_id, K.short_name, sum(bal_qty) as bal_qty from 
            (select H.*, I.distributor_name from 
            (select F.*, G.sale_qty, ifnull(F.tot_qty,0)-ifnull(G.sale_qty,0) as bal_qty from 
            (select E.distributor_id, E.product_id, sum(tot_qty) as tot_qty from 
            (select C.distributor_id, C.product_id, ifnull(C.qty_in,0)-ifnull(D.qty_out,0) as tot_qty from 
            ...
            group by J.distributor_id, J.distributor_name, J.product_id, K.short_name 
            order by J.distributor_name, J.product_id";
        $query=$this->db->query($sql);
        return $query->result();
    }
    ...
    function get_box_stock_for_distributor() {
        $sql="select J.distributor_id, J.distributor_name, K.short_name, sum(J.bal_qty) as bal_qty from 
            ...
            (select * from box_master where status = 'Approved') K 
            on (J.box_id=K.id) 
            group by J.distributor_id, J.distributor_name, K.short_name 
            order by J.distributor_id, K.short_name";
        $query=$this->db->query($sql);
        return $query->result();
    }
