...
public function send_exception_report() {
    ...
    //date(A.date_of_processing)<date('".$date."') and 
    //date(A.date_of_deposit)<date('".$date."') and

    $sql = "select 'Sales' as temp_col, sum(case when date(A.created_on)=date('".$date."') then 1 else 0 end) as entry_done, 
        sum(case when  (A.status = 'Pending' or A.status = 'Deleted') and (A.delivery_status='Pending' or A.delivery_status='GP Issued' or A.delivery_status='Delivered Not Complete' or A.delivery_status='Delivered') then 1 else 0 end) as pending, 
            sum(
            case 
            when  (A.modified_approved_date is not null and date(A.modified_approved_date)=date('".$date."') and (date(A.created_on)=date('".$date."')  AND (A.ref_id is not null or A.ref_id != '')))
                 then 1 
            when (A.modified_approved_date is not null and date(A.modified_approved_date)=date('".$date."') and (date(A.created_on)=date('".$date."')  AND (A.ref_id is null or A.ref_id = '')))
                 then 0
            when  (A.modified_approved_date is not null and date(A.modified_approved_date)=date('".$date."') and (date(A.created_on)<date('".$date."') ))
                  then 1
            else 0
            END
            ) as prior_pending 
            ...
            sum(case when  (A.status = 'Pending' or A.status = 'Deleted')  then 1 else 0 end) as pending, 
             sum(
                case 
                when  (A.modified_approved_date is not null and date(A.modified_approved_date)=date('".$date."') and (date(A.created_on)=date('".$date."')  AND (A.ref_id is not null or A.ref_id != '')))
                     then 1 
            ...
            sum(case when  (A.status = 'Pending' or A.status = 'Deleted') and (delivery_status='Pending' or delivery_status='GP Issued' or delivery_status='Delivered Not Complete' or delivery_status='Delivered') and (A.ref_id is null or A.ref_id = '') then 1 else 0 end) as pending, 
           sum(
            case 
            when  (A.modified_approved_date is not null and date(A.modified_approved_date)=date('".$date."') and (date(A.created_on)=date('".$date."')  AND (A.ref_id is not null or A.ref_id != '')))
                 then 1 
    ...
    $sql = "select Distinct 'Sales' as temp_col, A.id, A.date_of_processing as ref_date, A.invoice_no as ref_no, A.distributor_id, A.invoice_amount as amount, A.modified_by, A.modified_on, B.distributor_name, concat(ifnull(C.first_name,''),' ',ifnull(C.last_name,'')) as modifiedby 
        ...
        where A.date_of_processing is not null and A.distributor_id!='1' and A.distributor_id!='189' and (A.modified_approved_date is not null and date(A.modified_approved_date)=date('".$date."') and 
            (Case When 
        ...
        where A.date_of_deposit is not null  and (A.modified_approved_date is not null and date(A.modified_approved_date)=date('".$date."') and 
            (Case When 
            date(A.created_on)=date('".$date."')  AND (A.ref_id is not null or A.ref_id != '')
            Then 1=1
            When date(A.created_on)=date('".$date."')  AND (A.ref_id is null or A.ref_id = '')
        ...
        where A.date_of_processing is not null  and (A.modified_approved_date is not null and date(A.modified_approved_date)=date('".$date."') and (Case When 
            date(A.created_on)=date('".$date."')  AND (A.ref_id is not null or A.ref_id != '')
            Then 1=1
            When date(A.created_on)=date('".$date."')  AND (A.ref_id is null or A.ref_id = '')
            Then 1=0
        ...
        from distributor_out A left join distributor_master B on (A.distributor_id=B.id) left join user_master C on (A.modified_by=C.id) 
        where A.date_of_processing is not null and (A.distributor_id='1' or A.distributor_id='189')  and (A.modified_approved_date is not null and date(A.modified_approved_date)=date('".$date."') and (Case When 
            date(A.created_on)=date('".$date."')  AND (A.ref_id is not null or A.ref_id != '')
            Then 1=1
            When date(A.created_on)=date('".$date."')  AND (A.ref_id is null or A.ref_id = '')
            Then 1=0
        ...
        where A.date_of_transaction is not null and (A.modified_approved_date is not null and date(A.modified_approved_date)=date('".$date."') and (Case When 
            date(A.created_on)=date('".$date."')  AND (A.ref_id is not null or A.ref_id != '')
            Then 1=1
            When date(A.created_on)=date('".$date."')  AND (A.ref_id is null or A.ref_id = '')
            Then 1=0
