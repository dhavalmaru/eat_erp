...
function get_sales_rep1_email($frequency=''){
    ...
    $sql = "select count(distinct(m.sales_rep_id)) as total_merchandiser from merchandiser_beat_plan m left join sales_rep_master s on m.sales_rep_id=s.id where (s.status='Approved' or s.status='Active') and m.frequency='".$frequency."'";
    $query=$this->db->query($sql);
    return $query->result();
}
