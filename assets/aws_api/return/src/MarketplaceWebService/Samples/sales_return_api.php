...
$date = date('Y-m-d');
$time = date('H:i:s', time() - 3600);
$EndDate = $date."T".$time."Z";
$request->setEndDate($EndDate);

// $request->setStartDate(new DateTime('-3 months', new DateTimeZone('UTC')));
...
$request2->setAvailableFromDate($StartDate);
$request2->setAvailableToDate($EndDate);
// $request2->setReportRequestIdList($ReportRequestId);
$request2->setAcknowledged(false);
$request2->setMWSAuthToken('529421203372');
$ReportRequestId = '113126018104';
...
// $request3->setReportId('17162690902018104');
  ...
  function invokeGetReport(MarketplaceWebService_Interface $service, $request) 
  {
      try {
              ...
              for($i=0; $i<count($order_data); $i++) {
                  ...
                  $sql = "select * from distributor_in where order_no='".$order_no."'";
