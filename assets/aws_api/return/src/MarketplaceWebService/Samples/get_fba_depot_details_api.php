...
$request2->setAvailableToDate(new DateTime('+2 days', new DateTimeZone('UTC')));
$request2->setAvailableFromDate(new DateTime('-1 months', new DateTimeZone('UTC')));
              ...
              // echo $ReportRequestId.'<br/><br/>';
                ...
                if ($response->isSetGetReportListResult()) { 
                    ...
                    foreach ($reportInfoList as $reportInfo) {
                        ...
                        // $report_processing_status = $reportInfo->getReportProcessingStatus();
  ...
  function invokeGetReport(MarketplaceWebService_Interface $service, $request) 
  {
      try {
              ...
              for($i=1; $i<count($arr); $i++) {
                  ...
                  $order_no = $arr[$i][0];
                  $order_item_id = $arr[$i][4];
                  $fulfillment_center_id = $arr[$i][45];

                  $index = '';
                  for($j=0; $j<count($order_data); $j++) {
                      if($order_data[$j]['order_no']==$order_no && $order_data[$j]['order_item_id']==$order_item_id) {
                          $index = $j;
                          break;
                      }
                  }
                  if($index===''){
                      ...
                      $order_data[$index]['order_no']=$order_no;
                      $order_data[$index]['order_item_id']=$order_item_id;
              ...
              //     $sql = "select * from fc_details where order_id='".$order_no."'";
              ...
              //     $sql = "insert into fc_details (order_id, fc_id, status, remarks, created_by, created_on, modified_by, modified_on, approved_by, approved_on, rejected_by, rejected_on) VALUES ('".$order_no."', '".$fulfillment_center_id."', 'Approved', 'This is system generated entry.', '".$curusr."', '".$now."', '".$curusr."', '".$now."', Null, Null, Null, Null)";
              //     $con->query($sql);
              //     echo $sql.'<br/><br/>';
              //     echo '<br/><br/>';
              // }
              ...
              for($i=0; $i<count($order_data); $i++) {
                  $order_no = $order_data[$i]['order_no'];
                  $order_item_id = $order_data[$i]['order_item_id'];
                  $fulfillment_center_id = $order_data[$i]['fulfillment_center_id'];

                  $sql = "select * from fc_details where order_id='".$order_no."' and order_item_id='".$order_item_id."'";
                  $result = $conn->query($sql);
                  if ($result->num_rows > 0) {
                    $row_arr = $result->fetch_all(MYSQLI_ASSOC);
                    if(count($row_arr)>0) {
                      $result->free();
                      echo 'order_no '.$order_no.' & order_item_id '.$order_item_id.' already exist.<br/><br/>';
                      continue;
                    }
                  }

                  $sql = "insert into fc_details (order_id, order_item_id, fc_id, status, remarks, created_by, created_on, modified_by, modified_on, approved_by, approved_on, rejected_by, rejected_on) VALUES ('".$order_no."', '".$order_item_id."', '".$fulfillment_center_id."', 'Approved', 'This is system generated entry.', '".$curusr."', '".$now."', '".$curusr."', '".$now."', Null, Null, Null, Null)";
                  $conn->query($sql);
                  echo $sql.'<br/><br/>';
                  echo '<br/><br/>';
              }
